...
deterministic. Fixtures are the real decorated gamertags from a user's vault
...
@pytest.fixture
def temp_vault_db(tmp_path, monkeypatch):
    """Point the alias functions at a throwaway DB with the real schema."""
    db = tmp_path / "vault_data.sqlite"
    monkeypatch.setattr(bt, "VAULT_DB_PATH", str(db))
...
def test_alias_learn_then_resolve(temp_vault_db):
...
def test_alias_fuzzy_tolerates_ocr_drift(temp_vault_db):
...
def test_strong_direct_match_wins_over_alias(temp_vault_db):
...
def test_alias_ignored_when_player_left_roster(temp_vault_db):
    bt.learn_alias(ALLIANCE, GIBBERISH, 1)
...
def test_alias_scoped_per_alliance(temp_vault_db):
    bt.learn_alias(ALLIANCE, GIBBERISH, 1)
    assert bt.resolve_against_roster(GIBBERISH, DECORATED_ROSTER, 9999) == []


def test_blank_or_tiny_keys_are_not_learned(temp_vault_db):
    bt.learn_alias(ALLIANCE, "", 1)
    bt.learn_alias(ALLIANCE, "✦", 1)  # folds to empty/<2 chars
    with sqlite3.connect(temp_vault_db) as conn:
        count = conn.execute("SELECT COUNT(*) FROM vault_name_alias").fetchone()[0]
    assert count == 0


# ---------------------------------------------------------------------------
# Vault channel info message — self-heal fingerprint must match the rendered text
# ---------------------------------------------------------------------------
...
def test_info_message_fingerprints_present_in_render():
    ...
    rendered = bt.render_vault_info_message()
    assert bt._VAULT_INFO_FINGERPRINTS
    for fp in bt._VAULT_INFO_FINGERPRINTS:
