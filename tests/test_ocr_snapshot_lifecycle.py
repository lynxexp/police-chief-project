...
def _vault_session(finalize_fails=False):
    s = bt.VaultSession.__new__(bt.VaultSession)
    s.finalized = False
    s.lock = asyncio.Lock()
    s.channel_id = 1
    s.user_id = 2
...
def test_vault_failed_finalize_keeps_snapshot():
    s, calls = _vault_session(finalize_fails=True)
    with pytest.raises(RuntimeError):
        asyncio.run(s.finalize())
    assert calls.deleted == 0, "snapshot must survive a failed finalize for crash-resume"


def test_vault_successful_finalize_deletes_snapshot():
    s, calls = _vault_session()
    asyncio.run(s.finalize())
    assert calls.deleted == 1


def test_vault_save_snapshot_noop_after_finalize(monkeypatch):
    s, _ = _vault_session()
    s.finalized = True
    saved = []
    monkeypatch.setattr(ocr_resume, "save", lambda *a, **k: saved.append(a))
    s._snapshot_key = lambda: "vault:1:2"
    s.save_snapshot()
    assert saved == [], "a finalized session must not re-create its snapshot"
