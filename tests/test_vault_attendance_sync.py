...
def test_sync_writes_present_rows_points_equal_damage(tmp_path, monkeypatch):
    ...
    parsers.sync_vault_attendance_event(
        alliance_id=7, hunt_id=42, date="2026-07-05", trap_number=2,
    ...
    rows = conn.execute(
        ...
        "WHERE session_id='vault-42' ORDER BY player_id").fetchall()
    assert rows == [("1", "present", 100, "vault"), ("2", "present", 50, "vault")]
    sess = conn.execute(
        "SELECT event_type, event_time, origin, event_subtype, awaiting_result "
        "FROM attendance_sessions WHERE session_id='vault-42'").fetchone()
    assert sess == ("vault", "23:00", "vault", "Vault Trap 2", 0)
    name_row = conn.execute(
        "SELECT DISTINCT session_name FROM attendance_records "
        "WHERE session_id='vault-42'").fetchone()
    assert name_row[0] == "Vault Trap 2 - 2026-07-05"
    conn.close()
...
def test_sync_is_idempotent_and_rewrites_rows(tmp_path, monkeypatch):
    ...
    ids = [r[0] for r in conn.execute(
        "SELECT player_id FROM attendance_records WHERE session_id='vault-42'").fetchall()]
    assert ids == ["2"]   # rewritten to the second call's participants
    n = conn.execute(
        "SELECT COUNT(*) FROM attendance_sessions WHERE session_id='vault-42'").fetchone()[0]
    assert n == 1
    et = conn.execute(
        "SELECT event_time FROM attendance_sessions WHERE session_id='vault-42'").fetchone()[0]
    assert et is None
    conn.close()
...
def test_delete_removes_session_and_rows(tmp_path, monkeypatch):
    db = _mk_db(tmp_path, monkeypatch)
    parsers.sync_vault_attendance_event(
        alliance_id=7, hunt_id=42, date="2026-07-05", trap_number=1,
        event_time=None, alliance_name="SIR",
        participants=[{"fid": 1, "name": "Alice", "damage": 100}])
    parsers.delete_vault_attendance_event(hunt_id=42)
    conn = sqlite3.connect(db)
    assert conn.execute(
        "SELECT COUNT(*) FROM attendance_records WHERE session_id='vault-42'").fetchone()[0] == 0
    assert conn.execute(
        "SELECT COUNT(*) FROM attendance_sessions WHERE session_id='vault-42'").fetchone()[0] == 0
    conn.close()
