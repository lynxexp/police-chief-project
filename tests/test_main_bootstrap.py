...
catches the ImportError. This fork has no upstream release feed, so by default
it just explains the install looks incomplete and exits; only if the operator
opts in via `BOT_BOOTSTRAP_URL` does it download a source archive and extract
it, stripping the zip's wrapping top-level directory (e.g. `bot-main/cogs/foo.py`
...
def test_env_var_overrides_default_url(monkeypatch):
    """`BOT_BOOTSTRAP_URL` is the only source for the zip URL - this fork has
    no upstream release feed to fall back to. We can't exercise the except
    branch without re-deleting cogs/, but we can at least verify the lookup
    pattern main.py uses resolves to our value, and that the unset case
    resolves to None rather than a hard-coded URL."""
    ...
    resolved = os.environ.get("BOT_BOOTSTRAP_URL")
...
def test_no_env_var_exits_without_network_fetch(tmp_path, monkeypatch, capsys):
    """With no BOT_BOOTSTRAP_URL set, main.py must not fetch any URL (there is
    no upstream to fetch from) - it should just explain the install looks
    incomplete and exit."""
    install_dir = tmp_path / "install"
    install_dir.mkdir()
    monkeypatch.chdir(install_dir)
    ...
    with pytest.raises(SystemExit) as exc:
        main._bootstrap_from_main_branch(None)

    assert exc.value.code == 1
    out = capsys.readouterr().out
    assert "incomplete install" in out.lower()
    assert "whiteout" not in out.lower()
