...
def test_ensure_event_time_column_idempotent(tmp_path):
    ...
    conn.execute(
        "CREATE TABLE vault_hunts (id INTEGER PRIMARY KEY AUTOINCREMENT, "
        "alliance_id INTEGER, date TEXT, trap_number INTEGER, rallies INTEGER, "
        "total_damage INTEGER)"
    )
    conn.commit()
    bt._ensure_vault_hunts_event_time(conn)   # adds
    bt._ensure_vault_hunts_event_time(conn)   # no-op, must not raise
...
def test_vault_event_type_display():
    assert attendance.event_type_display("vault") == ("Vault Trap", "🔐")
