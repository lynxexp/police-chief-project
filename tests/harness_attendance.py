...
layer-2 tests run real OCR via vault_track on PNG fixtures.
"""
...
def _load_modules():
    """Load the attendance OCR parser module and vault_track (for real OCR).
    ...
    return parsers, vault_track


parsers, vault_track = _load_modules()
...
def ocr_image(path: Path, lang: str = "en") -> str:
    ...
    return asyncio.run(vault_track.ocr_bytes(data, lang=lang))
