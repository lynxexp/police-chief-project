...
# NOTE: a `test_classify_event_against_real_ocr_fixtures` test used to live
# here, verifying classify_event() against every fixture's expected
# (event_type, kind). It's gone: these fixtures are all screenshots of
# foundry_battle/canyon_clash/power_rankings/alliance_showdown events, and their
# fingerprint configs were deliberately removed from EVENT_TYPES (Phase 6) —
# those events are manual-entry only for Police Chief until real screenshots
# arrive. The fixtures themselves stay (test_fixture_extraction above still
# exercises the still-generic row/legion/date parsers against them), but
# there is currently no registered OCR event type any of them would
# correctly classify as, so the classification assertion has nothing
