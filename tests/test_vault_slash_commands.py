"""/vault_player_history and /vault_damage_view must not die after deferring.
...
bt = importlib.import_module("cogs.vault_track")
...
def _mk_cog():
    ...
    vault.execute("CREATE TABLE vault_hunts (id INTEGER PRIMARY KEY, alliance_id INTEGER, trap_number INTEGER, date TEXT)")
    vault.execute("CREATE TABLE vault_player_damage (hunt_id INTEGER, fid INTEGER, damage INTEGER, rank INTEGER)")

    cog = bt.VaultTrack.__new__(bt.VaultTrack)
    cog.users_cursor = users.cursor()
    cog.alliance_cursor = alliance.cursor()
    cog.vault_cursor = vault.cursor()

    async def allow(*a, **k):
        return True

    cog.check_vault_permission = allow
    return cog
...
def test_player_history_without_records_replies(monkeypatch):
    ...
    asyncio.run(bt.VaultTrack.vault_player_history.callback(
        cog, inter, alliance="5", player="7", trap_number=None))
...
def test_damage_view_rejects_malformed_date(monkeypatch):
    ...
    monkeypatch.setattr(bt, "VaultDamageView", _StubView)
    cog.data_submit = SimpleNamespace(process_view=fake_process_view)

    asyncio.run(bt.VaultTrack.vault_damage_view.callback(
        cog, inter, alliance="5", trap_number=None,
