...
def test_register_rejects_unknown_alliance(monkeypatch):
    ...
    asyncio.run(ar.AllianceRegistration.register.callback(cog, inter, fid=7, alliance=999, name="Bob"))
...
def test_register_accepts_known_alliance(monkeypatch):
    ...
    asyncio.run(ar.AllianceRegistration.register.callback(cog, inter, fid=7, alliance=5, name="Bob"))
