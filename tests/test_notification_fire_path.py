...
def _row(tz_name, minutes_ahead=30):
    ...
    #  instance_identifier, custom_delete_delay_minutes, custom_event_id)
    return (1, 10, 20, 12, 0, tz_name, "Test event", 1, "none", 0, 0, 1,
            None, None, None, next_at, None, None, None, None)


def test_dedupe_suppresses_duplicate_in_non_utc_timezone():
    chan = _Chan()
    cog, _ = _mk_cog(chan)
    cog.cursor.execute("INSERT INTO vault_notifications (id) VALUES (1)")
    cog.conn.commit()
...
def test_successful_send_remembers_channel_name():
    ...
    stored = cog.cursor.execute(
        "SELECT channel_name FROM vault_notifications WHERE id = 1").fetchone()[0]
    assert stored == "vault-trap", "sends must refresh the last-known channel name"
...
def test_pause_names_the_deleted_channel():
    ...
    cog.cursor.execute(
        "INSERT INTO vault_notifications (id, is_enabled, description, guild_id, event_type,"
        " hour, minute, timezone, channel_id, channel_name)"
        " VALUES (1, 1, 'EMBED_MESSAGE:x', 10, 'Vault Trap', 14, 0, 'UTC', 20, 'vault-trap')")
    cog.conn.commit()
    dms = []
    ...
    assert paused == 1
    assert dms[0]["channel_label"] == "#vault-trap (deleted)", \
    ...
    assert "Vault Trap 14:00 (UTC)" in line and "(Embed notification)" in line


def test_paused_line_is_human_readable():
    line = ns._format_paused_line("Vault Trap", 14, 0, "UTC", "EMBED_MESSAGE:whatever")
    assert line == "- **Vault Trap 14:00 (UTC)** - (Embed notification)"
