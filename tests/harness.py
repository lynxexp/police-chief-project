"""Test harness wrapping the vault-track parsing pipeline.

Imports `bot/cogs/vault_track.py` directly so changes to the bot's parsing
...
def _load_vault_track():
    """Load `vault_track` as a member of the `cogs` package so its relative
    ...
    # `import cogs.vault_track` triggers the package's relative imports
    # cleanly, including pimp_my_bot etc.
    import cogs.vault_track as vault_track  # type: ignore
    return vault_track


bt = _load_vault_track()


def _load_capitol_war():
    """Load `capitol_war` the same way `_load_vault_track` loads vault_track
    -- as a member of the `cogs` package, so its relative imports (including
    `from .vault_track import ...`) resolve without the discord.py runtime."""
    if str(BOT_DIR) not in sys.path:
        sys.path.insert(0, str(BOT_DIR))
    import cogs.capitol_war as capitol_war  # type: ignore
    return capitol_war


ct = _load_capitol_war()
...
def parse_text(ocr_text: str, roster: Optional[list] = None) -> dict[str, Any]:
    ...
    trap, rallies, total_damage = bt.extract_vault_hunt_stats(repaired)
...
def _ocr_bytes_sync(image_bytes: bytes, lang: str) -> str:
    """vault_track.ocr_bytes is async in the current bot; run it to completion
...
def run_ocr_pipeline(image_bytes: bytes, primary_lang: str = "ch",
    ...
    trap, rallies, total = bt.extract_vault_hunt_stats(repaired)
