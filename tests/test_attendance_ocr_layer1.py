...
#
# The foundry_battle / canyon_clash / power_rankings / alliance_showdown
# fingerprint configs were deliberately removed from EVENT_TYPES (Phase 6):
# no Police Chief screenshots exist yet for their would-be equivalents (PD
# Development, Arms Race, etc.), so those events are manual-entry only for
# now — see attendance_ocr_parsers.py's Phase-6 note above EVENT_TYPES.
# vault_trap is the one real, currently-registered OCR event, so the
# classifier-mechanism tests below exercise it directly instead of the
# removed fingerprints.
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("text,expected", [
    ("Vault Trap Operation results", True),
    ("Battle Overview for this week", True),
    ("Solo Damage Rewards distributed", True),
    ("Damage Rankings top 10", True),
    ("Damage Ranking top 10", True),  # singular also matches (Rankings? )
    ("Canyon Clash page", False),
    ("just player rows: vtr 123 H&N 456", False),
])
def test_fingerprint_match(text, expected):
    assert parsers.fingerprint_match("vault_trap", text) == expected


@pytest.mark.parametrize("kind,expected", [
    ("result", True),          # vault_trap's only registered kind
    ("registration", False),   # no such kind exists for vault_trap
])
def test_fingerprint_match_by_kind(kind, expected):
    assert parsers.fingerprint_match("vault_trap", "Vault Trap Operation", kind=kind) == expected
...
@pytest.mark.parametrize("text,expected_kind", [
    ("Vault Trap Operation results", "result"),
    ("Battle Overview", "result"),
    ("Solo Damage Rewards", "result"),
])
def test_detect_kind(text, expected_kind):
    assert parsers.detect_kind("vault_trap", text) == expected_kind


def test_detect_kind_returns_none_for_scroll_page():
    """Continuation pages have no header → no fingerprint match → None."""
    assert parsers.detect_kind("vault_trap", "just player rows: vtr 123 H&N 456") is None
...
_TEXT_VAULT_RESULT = "Vault Trap Operation - Battle Overview and Damage Rankings"
_ALL_EVENTS = ["vault_trap"]


def test_classify_event_no_keywords_fingerprint_alone_works():
    assert parsers.classify_event(_TEXT_VAULT_RESULT, _ALL_EVENTS) == ("vault_trap", "result")


def test_classify_event_disabled_event_never_classified():
    assert parsers.classify_event(_TEXT_VAULT_RESULT, []) is None  # vault_trap deliberately omitted
...
# ---------------------------------------------------------------------------
# Event-date weekday snap. No currently-registered event type sets
# event_weekday (vault_trap's cadence is unconfirmed for Police Chief, so
# it's left None - see the EVENT_TYPES note in attendance_ocr_parsers.py).
# The snap-to-weekday mechanism itself is still generic/reusable machinery
# for whenever a real cadence is confirmed, so it's exercised here via a
# synthetic weekday-configured event registered through monkeypatch. ±1 day
# from target → adjusted; anything further → mismatch and the date is
# returned as-is.
# ---------------------------------------------------------------------------

@pytest.fixture
def weekday_event(monkeypatch):
    """Registers a throwaway event type pinned to Sunday (weekday=6)."""
    cfg = parsers.EventTypeConfig(
        key="_test_weekday_event",
        label="Test Weekday Event",
        default_keywords=(),
        fingerprint_re_by_kind={},
        event_weekday=6,
    )
    monkeypatch.setitem(parsers.EVENT_TYPES, "_test_weekday_event", cfg)
    return "_test_weekday_event"


def test_resolve_event_date_exact_on_target_weekday(weekday_event):
    # 2026-05-03 is a Sunday
    resolved, conf = parsers.resolve_event_date(date(2026, 5, 3), weekday_event)
    assert resolved == date(2026, 5, 3)
    assert conf == "exact"


def test_resolve_event_date_adjusted_from_monday(weekday_event):
    # 2026-05-04 is a Monday — snap back to Sunday 2026-05-03
    resolved, conf = parsers.resolve_event_date(date(2026, 5, 4), weekday_event)
    assert resolved == date(2026, 5, 3)
    assert conf == "adjusted"


def test_resolve_event_date_adjusted_from_saturday(weekday_event):
    # 2026-05-02 is a Saturday — snap forward to Sunday 2026-05-03
    resolved, conf = parsers.resolve_event_date(date(2026, 5, 2), weekday_event)
    assert resolved == date(2026, 5, 3)
    assert conf == "adjusted"


def test_resolve_event_date_mismatch_when_far(weekday_event):
    # 2026-04-29 (Wednesday) is too far from any Sunday → mismatch
    resolved, conf = parsers.resolve_event_date(date(2026, 4, 29), weekday_event)
    assert resolved == date(2026, 4, 29)
    assert conf == "mismatch"


def test_resolve_event_date_event_without_weekday_returns_exact():
    # vault_trap has no event_weekday configured — always exact
    resolved, conf = parsers.resolve_event_date(date(2026, 5, 14), "vault_trap")
...
# This is the algorithm vault_track has had for ages; we now mirror it.
# ---------------------------------------------------------------------------
