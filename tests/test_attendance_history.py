...
def test_event_label_other_shows_session_name():
    ...
    # Registered fingerprint-classifier event types (attendance_ocr_parsers.EVENT_TYPES)
    # ignore session_name and keep their own configured label.
    assert ah._event_label("vault_trap", "Legion 1", "x") == "Vault Trap · Legion 1"


def test_event_label_vault_track_source_shows_vault_trap():
    """Vault-track-sourced attendance rows store the internal key 'vault'
    (see attendance_ocr_parsers.sync_vault_attendance_event) - not a key in
    attendance_ocr_parsers.EVENT_TYPES (that dict's only entry, 'vault_trap',
    is a different, currently-unused generic-OCR fingerprint config). Without
    falling back to attendance.event_type_display, the history view would
    show the raw internal key 'vault' instead of 'Vault Trap'."""
    assert ah._event_label("vault", None) == "Vault Trap"
    assert ah._event_label("vault", "Vault Trap 2") == "Vault Trap · Vault Trap 2"


def test_event_label_manual_event_passes_through():
    """Manual (non-OCR) entries already store their display name directly."""
    assert ah._event_label("PD Development", None) == "PD Development"
