...
Automated regression tests for the bot: pure-logic suites (event /
...
suites, and a Python 3.11 f-string compatibility scan.
...
so it ships to **no** user via release archives — yet stays versioned with the
code it tests. Bot Health's cleanup also removes a stray `tests/` from any
install.
...
├── harness.py                          # imports cogs/vault_track.py — vault-hunt parser pipeline
├── harness_attendance.py               # imports cogs/attendance_ocr_parsers.py — attendance OCR pipeline
├── test_layer1_parser.py               # pure-text vault-track tests
├── test_layer2_ocr.py                  # end-to-end vault-track OCR tests (slow)
...
├── fixtures/                           # vault-track + attendance/ screenshots, expected JSON, rosters
└── README.md
```
...
```
python -m pytest tests/test_layer1_parser.py -v          # vault-track layer 1
python -m pytest tests/test_layer2_ocr.py -v             # vault-track layer 2 (slow)
...
## Adding a new vault-track fixture
