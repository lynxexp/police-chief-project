...
# 15 min idle before auto-finalize, matching vault_track; admins read counts before deciding.
...
# NOTE (Phase 6): the foundry_battle / canyon_clash / power_rankings /
# alliance_showdown entries were removed here -- their fingerprint regexes were
# tuned to screenshot text (e.g. "Foundry Battle", "Personal Arsenal
# Points") that will never appear in a Police Chief screenshot, and no
# Police Chief screenshots exist yet for their would-be equivalents (PD
# Development, Arms Race, Officer Program, Hero Program, Alliance Faceoff,
# State Capitol Siege). Those event names still work fine for manual entry
# (see attendance.py EVENT_TYPES) -- they just have no OCR auto-detection
# until real screenshots are supplied. FoundryBattleSession / CanyonClashSession
# / AllianceShowdownSession below are left defined but unreferenced by
# SESSION_CLASSES for the same reason (harmless dead code, not deleted in case
# their parsing helpers are useful once real screenshots arrive).
#
# vault_trap is Police Chief's own OCR-supported event (real screenshots -- see
# docs/ocr-reference/vault_trap.md). Unlike the entries above, it is NOT routed
# through this generic multi-image pipeline (SESSION_CLASSES / attendance_ocr.py) --
# Vault Trap has its own dedicated pipeline in vault_track.py (mail detection,
# name-fuzzy-matching, persistence, leaderboard rendering) with its own
# channel setup, kept separate because its
# screenshot format (paginated rankings, no per-row alliance tag, medal-badge
# top-3 ranks) doesn't fit the shared OcrUploadSession review flow. This entry
# exists so vault_track.py can reuse one shared fingerprint/regex definition
# instead of duplicating it; attendance_ocr_setup.py explicitly excludes
# vault_trap from its generic per-channel event toggle (see
# _TOGGLEABLE_EVENT_TYPES there) so it can never be enabled for the wrong
# pipeline and hit "Parser not implemented".
EVENT_TYPES: dict[str, EventTypeConfig] = {
    "vault_trap": EventTypeConfig(
        key="vault_trap",
        label="Vault Trap",
        default_keywords=("Vault Trap Operation", "Vault Trap"),
        fingerprint_re_by_kind={
            # Single mail, no separate registration phase -- everything is "result".
            "result": re.compile(
                r"Vault\s+Trap\s+Operation"
                r"|Battle\s+Overview"
                r"|Solo\s+Damage\s+Rewards"
                r"|Damage\s+Rankings?",
                re.IGNORECASE,
            ),
        },
        # Cadence/weekday unconfirmed for Police Chief -- left None rather than guessed.
        event_weekday=None,
        legion_required=False,
    ),
}
...
# editor is always offered (so a missed panel can be filled by hand). Empty for
# now -- no currently-registered event type (see EVENT_TYPES) uses this panel.
STATS_PANEL_EVENTS: frozenset[str] = frozenset()

# Allowed event-time slots (UTC). Mirrors notification_event_types.EVENT_CONFIG.
# Empty -- no confirmed Police Chief cadence data yet (see EVENT_TYPES note).
...
def _alliance_ocr_langs(alliance_id, names) -> tuple[str, list]:
    """Primary + fallback OCR languages for the alliance. Shared with Vault
    Trap Tracking's OCR-language setting (one place per alliance). Fallbacks are
    auto-derived from `names` when auto-manage is on, else the manual config."""
    from . import vault_track
    default = vault_track.DEFAULT_OCR_LANG
    try:
        with sqlite3.connect("db/alliance.sqlite", timeout=30.0) as conn:
            row = conn.execute(
                "SELECT vault_ocr_lang FROM alliancesettings WHERE alliance_id = ?",
    ...
    if primary not in vault_track.OCR_LANG_CODES:
        primary = default
    fbs = vault_track.auto_managed_fallbacks(alliance_id, names, primary=primary)
    return primary, fbs
...
async def ocr_value_rows(image_bytes: bytes, *, roster, alliance_id, session=None, parse=None, primary_text: str | None = None, progress_callback=None):
    ...
    if not fallbacks or not roster:
        text = primary_text if primary_text is not None else await vault_track.ocr_bytes(image_bytes, lang=primary, session=session)
        rows = parse(vault_track.repair_ocr_digits(text)) if text.strip() else []
        return rows, text
    ...
    def merge(rows, fb_rows, fb_text, _lang, *, primary_boxed=None, fb_boxed=None):
        ...
        if any(is_unfilled(r) for r in rows) and primary_boxed and fb_boxed:
            ...
            for r in rows:
                ...
                by_damage[r["value"]] = r
            try:
                vault_track.merge_fallback_rows_by_boxes(
                    by_damage, primary_boxed, fb_boxed, roster, _lang)
            finally:
                for r in rows:
                    r.pop("damage", None)
        # Non-Latin engines read names well but mangle numbers, so value-merge
        # usually misses them. Fall back to vault_track's anchor-based position fill
        ...
        if any(is_unfilled(r) for r in rows):
            ...
            try:
                ...
                vault_track.fill_unfilled_by_position(
                    {id(r): r for r in rows}, fb_text, _lang, "attendance", roster)
            finally:
                for r in rows:
                    r.pop("damage", None)

    return await vault_track.ocr_rows_with_fallback(
...
def load_alliance_roster(alliance_id: int) -> list[tuple[int, str]]:
    """Active-only (decision #2): attendance OCR name-matching should only
    match against current members, so a deactivated member's name in a new
    screenshot lands unmatched until they're reactivated."""
...
def _skeleton(s: str) -> str:
    ...
    their Latin equivalents, reusing vault_track's table so the two stay in sync.
    Bound lazily to respect the module's deferred vault_track import."""
    global _skeleton_fn
    if _skeleton_fn is None:
        from . import vault_track
        _skeleton_fn = vault_track._skeleton
...
def _vault_session_id(hunt_id) -> str:
    return f"vault-{hunt_id}"


def _vault_trap_label(trap_number) -> str:
    """Display label for the `trap_number` field: the Vault Trap
    operation/wave number (the "In Vault Trap {N}, you earned..." text).
    No confirmed Police Chief equivalent for "both" exists, so that special
    case is dropped rather than guessed."""
    return f"Vault Trap {trap_number}"


def sync_vault_attendance_event(*, alliance_id, hunt_id, date, trap_number,
                               event_time, alliance_name, participants) -> None:
    """Create or replace the attendance event mirroring one Vault Trap hunt
    (internal db keys use the 'vault' name -- see vault_track.py). Vault Trap
    tracking is the source of truth: the event's records are fully rewritten
    to the current participants (all 'present', points=damage). event_time
    may be None."""
    session_id = _vault_session_id(hunt_id)
    trap_label = _vault_trap_label(trap_number)
...
def delete_vault_attendance_event(*, hunt_id) -> None:
    """Remove the attendance event mirroring a deleted vault hunt."""
    session_id = _vault_session_id(hunt_id)
    with sqlite3.connect(_ATT_DB, timeout=30.0) as conn:
        conn.execute("DELETE FROM attendance_records WHERE session_id = ?", (session_id,))
...
class OcrUploadSession:
    ...
    def __init__(self, *, cog, channel: discord.TextChannel, uploader_id: int,
        ...
        # Progress counters mirror vault_track so the embed can show "image X of Y"
    ...
    async def _phase_callback(self, phase: str, lang: str):
        ...
        language pass is running (mirrors vault_track's indicator)."""
...
class _PointsSession(OcrUploadSession):
    ...
    async def _process_attachments(self, attachments: list[discord.Attachment]):
        ...
        for att in attachments:
            ...
            try:
                data = await att.read()
                await self._phase_callback('ocr', vault_track.DEFAULT_OCR_LANG)
                blocks = await vault_track.ocr_bytes_with_boxes(
                    data, lang=vault_track.DEFAULT_OCR_LANG)
...
# No event type is currently wired into this generic multi-image pipeline --
# see the NOTE above EVENT_TYPES. PowerRankingsSession / FoundryBattleSession /
# CanyonClashSession / AllianceShowdownSession are left defined above but
# unreferenced here (build_session/_from_snapshot already return None
# gracefully for unknown keys, so this is safe -- not a crash risk).
