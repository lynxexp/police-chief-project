...
from .vault_track import _isolate_rtl, _ltr_line
...
# meaningful attendance ratio. No Police Chief event has confirmed sign-up-phase
# OCR support yet (see attendance_no_shows.py) -- left empty rather than guessed.
_RATIO_EVENTS = set()
...
def _event_label(event_type: str, subtype: str | None, session_name: str | None = None) -> str:
    ...
    if cfg:
        ...
        # Not a fingerprint-classifier event (this dict only has "vault_trap",
        # the generic multi-image OCR entry). Vault-track-sourced rows store
        # the internal key "vault" (see attendance_ocr_parsers.sync_vault_attendance_event),
        # which attendance.py's event_type_display already knows how to turn
        # into "Vault Trap" - reuse that instead of falling back to the raw
        # internal key. Manual entries already store their display name
        # directly (e.g. "PD Development") and pass through unchanged.
        from .attendance import event_type_display
        label, _icon = event_type_display(event_type)
