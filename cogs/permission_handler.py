...
class PermissionManager:
    ...
    @staticmethod
    def can_manage_alliance(user_id: int, guild_id: int, alliance_id: int) -> bool:
        """Pure predicate: is this user an admin (any tier) with access to
        this specific alliance -- i.e. Alliance Admin tier or higher for
        that alliance. This is the one shared definition of "alliance admin
        and up" used across the bot: vault_track.can_manage_vault /
        capitol_war.can_manage_capitol, the "manage" branch of both
        check_*_permission methods, and the admin_only_add/admin_only_view
        toggles (including gating the channel-listener screenshot-upload
        path itself, not just the manual /vault_damage_add and
        /capitol_add commands) all delegate here so the tier logic lives
