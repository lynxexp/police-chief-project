"""
Alliance history cog. Tracks and displays nickname and Chief's Office level changes.
"""
import discord
...
class _SingleHistoryResultView(discord.ui.View):
    ...
    def __init__(self, cog, alliance_id, history_type: str, embed: discord.Embed):
        ...
        self.history_type = history_type  # "chief_office", "nickname", "power", or "combat_power"
        self.embed = embed
    ...
    async def back_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.alliance_id is None:
            ...
            return
        if self.history_type == "chief_office":
            await self.cog.show_member_list_chief_office(interaction, self.alliance_id)
...
class AllianceHistory(commands.Cog):
        ...
        try:
            ...
            if not changes:
                await interaction.followup.send(
                    "No Chief's Office level changes found for this player.",
                    ephemeral=True
                )
                return
            ...
            with sqlite3.connect('db/users.sqlite') as users_db:
                ...
                cursor.execute(
                    "SELECT nickname, chief_office_lv, alliance FROM users WHERE fid = ?",
                    (fid,),
                )
            ...
            embed = discord.Embed(
                title=f"{theme.levelIcon} Chief's Office Level History",
                description=(
                    f"**Player:** `{nickname}`\n"
                    f"**ID:** `{fid}`\n"
            ...
            await interaction.followup.send(
                embed=embed,
                view=_SingleHistoryResultView(self, alliance_id, "chief_office", embed),
                ephemeral=True,
            )

        except Exception as e:
            logger.error(f"Error in show_chief_office_history: {e}")
            print(f"Error in show_chief_office_history: {e}")
            await interaction.followup.send(
                f"{theme.deniedIcon} An error occurred while displaying the Chief's Office level history.",
                ephemeral=True
            )
    ...
    async def show_member_list_chief_office(self, interaction: discord.Interaction, alliance_id: int):
        """Show the member list for Chief's Office history selection (skip alliance picker)."""
        try:
            with sqlite3.connect('db/users.sqlite') as users_db:
                cursor = users_db.cursor()
                cursor.execute(
                    "SELECT fid, nickname, chief_office_lv FROM users "
                    "WHERE alliance = ? ORDER BY chief_office_lv DESC, nickname",
                    (alliance_id,),
                )
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"Select a member to view Chief's Office level history:\n"
                    f"{theme.upperDivider}\n"
            ...
            logger.error(f"Error in show_member_list_chief_office: {e}")
            print(f"Error in show_member_list_chief_office: {e}")
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"{theme.upperDivider}\n"
                    f"{theme.stoveIcon} **Chief's Office Changes**\n"
                    f"└ Track Chief's Office level changes over time\n\n"
    ...
    async def show_recent_changes(self, interaction: discord.Interaction, alliance_name: str, match: re.Match):
        ...
        try:
            ...
            with sqlite3.connect('db/changes.sqlite', timeout=30.0) as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT fid, old_chief_office_lv, new_chief_office_lv, change_date
                    FROM chief_office_changes
                    WHERE fid IN ({})
...
class HistoryTypeView(discord.ui.View):
    ...
    @discord.ui.button(label="Chief's Office Changes", emoji=f"{theme.stoveIcon}",
                       style=discord.ButtonStyle.primary, row=0)
    async def chief_office(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.cog.show_member_list_chief_office(interaction, self.alliance_id)
...
class MemberListView(discord.ui.View):
    ...
    def update_view(self):
        ...
        select = discord.ui.Select(
            ...
            options=[
                discord.SelectOption(
                    label=f"{name}",
                    value=str(fid),
                    description=f"ID: {fid} | Level: {self.cog.level_mapping.get(chief_office_lv, str(chief_office_lv))}"
                ) for fid, name, chief_office_lv in current_members
            ],
            row=0
        )
        ...
        async def member_callback(interaction):
            try:
                ...
                await self.cog.show_chief_office_history(interaction, fid)
            except Exception as e:
                logger.error(f"Error in member_callback: {e}")
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        f"{theme.deniedIcon} An error occurred while showing Chief's Office level history.",
                        ephemeral=True
                    )
                else:
                    await interaction.followup.send(
    ...
    async def search_callback(self, interaction: discord.Interaction):
        modal = ChiefOfficeHistoryIDSearchModal(self.cog)
        await interaction.response.send_modal(modal)
...
class ChiefOfficeHistoryIDSearchModal(discord.ui.Modal, title="Search by ID"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            logger.error(f"Error in ChiefOfficeHistoryIDSearchModal on_submit: {e}")
            print(f"Error in ChiefOfficeHistoryIDSearchModal on_submit: {e}")
            if not interaction.response.is_done():
                await interaction.response.send_message(
