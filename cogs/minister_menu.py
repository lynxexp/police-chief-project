"""
Shift scheduling menu. UI for managing appointment bookings and rotations for
admin-configurable appointment types.
"""
import discord
...
class UpdateNamesModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"{theme.settingsIcon} Shift Settings",
            description=(
                f"{theme.verifiedIcon} **{result_msg}**\n\n"
                f"Administrative settings for shift scheduling:\n\n"
                f"Available Actions\n"
                f"{theme.upperDivider}\n\n"
                f"{theme.editListIcon} **Appointment Types**\n"
                f"└ Add, rename, reorder or delete appointment types\n\n"
                f"{theme.editListIcon} **Update Names**\n"
                f"└ Manually set booked members' names\n\n"
...
class ActivitySelectView(discord.ui.View):
    def __init__(self, bot, cog, action_type):
        ...
        labels = self.cog.get_appointment_type_labels()
        if labels:
            options = [discord.SelectOption(label=label[:100], value=label) for label in labels[:25]]
            select = discord.ui.Select(placeholder="Select an appointment type...", options=options)
            select.callback = self.activity_select
        else:
            select = discord.ui.Select(
                placeholder="No appointment types configured yet",
                options=[discord.SelectOption(label="No appointment types", value="none")],
                disabled=True
            )
        self.add_item(select)

    async def activity_select(self, interaction: discord.Interaction):
...
class AddAppointmentTypeModal(discord.ui.Modal, title="Add Appointment Type"):
    def __init__(self, cog):
        super().__init__()
        self.cog = cog
        self.label_input = discord.ui.TextInput(
            label="Appointment Type Name",
            placeholder="e.g. Rally Lead, Patrol Shift",
            required=True,
            max_length=100
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        schedule_cog = self.cog.bot.get_cog("MinisterSchedule")
        if not schedule_cog:
            await interaction.response.send_message(f"{theme.deniedIcon} Shift Schedule module not found.", ephemeral=True)
            return
        if not label:
            await interaction.response.send_message(f"{theme.deniedIcon} Name cannot be empty.", ephemeral=True)
            return
        if label in schedule_cog.get_appointment_type_labels():
            await interaction.response.send_message(f"{theme.deniedIcon} `{label}` already exists.", ephemeral=True)
            return

        schedule_cog.add_appointment_type(label)
        await self.cog.show_appointment_types_menu(interaction, message=f"Added appointment type `{label}`.")

class RenameAppointmentTypeModal(discord.ui.Modal, title="Rename Appointment Type"):
    def __init__(self, cog, type_id, current_label):
        super().__init__()
        self.cog = cog
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        schedule_cog.rename_appointment_type(self.type_id, label)
        await self.cog.show_appointment_types_menu(interaction, message=f"Renamed to `{label}`.")

class DeleteAppointmentTypeConfirmView(discord.ui.View):
    def __init__(self, bot, cog, type_id, label):
        super().__init__(timeout=7200)
        self.bot = bot
        self.cog = cog
        self.type_id = type_id
    ...
    async def confirm_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        schedule_cog = self.bot.get_cog("MinisterSchedule")
        if schedule_cog:
            schedule_cog.delete_appointment_type(self.type_id)
        await self.cog.show_appointment_types_menu(interaction, message=f"Deleted appointment type `{self.label}`.")

    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary, emoji=f"{theme.deniedIcon}")
    async def cancel_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.cog.show_appointment_types_menu(interaction)

class AppointmentTypesView(discord.ui.View):
    """Admin CRUD screen for the alliance's appointment-type labels (add,
    rename, delete, reorder) - fully admin-defined, not a fixed list."""

    def __init__(self, bot, cog, selected_id: int = None):
        super().__init__(timeout=7200)
        self.bot = bot
        self.cog = cog
        ...
        schedule_cog = self.bot.get_cog("MinisterSchedule")
        self.types = schedule_cog.get_appointment_types() if schedule_cog else []  # [(id, label, sort_order), ...]

        if self.types:
            options = [
                discord.SelectOption(label=label[:100], value=str(tid), default=(tid == selected_id))
                for tid, label, _sort in self.types[:25]
            ]
            select = discord.ui.Select(placeholder="Select a type to rename, delete or reorder...", options=options)
        ...
        idx = next((i for i, t in enumerate(self.types) if t[0] == selected_id), None)
        has_selection = idx is not None

        add_btn = discord.ui.Button(label="Add", style=discord.ButtonStyle.success, emoji=f"{theme.editListIcon}", row=1)
        add_btn.callback = self.add_callback
        self.add_item(add_btn)

        rename_btn = discord.ui.Button(label="Rename", style=discord.ButtonStyle.primary, emoji=f"{theme.editListIcon}", row=1, disabled=not has_selection)
        rename_btn.callback = self.rename_callback
        self.add_item(rename_btn)

        delete_btn = discord.ui.Button(label="Delete", style=discord.ButtonStyle.danger, emoji=f"{theme.trashIcon}", row=1, disabled=not has_selection)
        delete_btn.callback = self.delete_callback
        self.add_item(delete_btn)

        up_btn = discord.ui.Button(
            label="Move Up", style=discord.ButtonStyle.secondary, emoji=f"{theme.prevIcon}", row=2,
            disabled=not has_selection or idx == 0
        )
        up_btn.callback = self.move_up_callback
        self.add_item(up_btn)

        down_btn = discord.ui.Button(
            label="Move Down", style=discord.ButtonStyle.secondary, emoji=f"{theme.nextIcon}", row=2,
            disabled=not has_selection or idx is None or idx == len(self.types) - 1
        )
        down_btn.callback = self.move_down_callback
        self.add_item(down_btn)

        back_btn = discord.ui.Button(label="Back", style=discord.ButtonStyle.primary, emoji=f"{theme.backIcon}", row=2)
        back_btn.callback = self.back_callback
        self.add_item(back_btn)

    async def select_callback(self, interaction: discord.Interaction):
        selected_id = int(interaction.data['values'][0])
        await self.cog.show_appointment_types_menu(interaction, selected_id=selected_id)

    async def add_callback(self, interaction: discord.Interaction):
        await interaction.response.send_modal(AddAppointmentTypeModal(self.cog))

    async def rename_callback(self, interaction: discord.Interaction):
        if self.selected_id is None:
            await interaction.response.send_message(f"{theme.deniedIcon} Select a type first.", ephemeral=True)
            return
        label = next((t[1] for t in self.types if t[0] == self.selected_id), "")
        await interaction.response.send_modal(RenameAppointmentTypeModal(self.cog, self.selected_id, label))
    ...
    async def delete_callback(self, interaction: discord.Interaction):
        if self.selected_id is None:
            ...
            return
        label = next((t[1] for t in self.types if t[0] == self.selected_id), "")
        await self.cog.show_delete_appointment_type_confirmation(interaction, self.selected_id, label)

    async def move_up_callback(self, interaction: discord.Interaction):
        schedule_cog = self.bot.get_cog("MinisterSchedule")
        if schedule_cog and self.selected_id is not None:
            schedule_cog.move_appointment_type(self.selected_id, "up")
        await self.cog.show_appointment_types_menu(interaction, selected_id=self.selected_id)

    async def move_down_callback(self, interaction: discord.Interaction):
        schedule_cog = self.bot.get_cog("MinisterSchedule")
        if schedule_cog and self.selected_id is not None:
            schedule_cog.move_appointment_type(self.selected_id, "down")
        await self.cog.show_appointment_types_menu(interaction, selected_id=self.selected_id)
...
class MinisterSettingsView(discord.ui.View):
        ...
        if not is_global:
            for child in self.children:
                if isinstance(child, discord.ui.Button) and child.label in [
                    "Appointment Types", "Schedule List Type", "Time Slot Mode",
                    "Delete All Reservations", "Clear Channels", "Delete Server ID"
                ]:
                    child.disabled = True

    @discord.ui.button(label="Appointment Types", style=discord.ButtonStyle.success, emoji=f"{theme.editListIcon}", row=0)
    async def appointment_types(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Check if user is global admin
        is_admin, is_global_admin, _ = await self.cog.get_admin_permissions(interaction.user.id)
        if not is_global_admin:
            await interaction.response.send_message(f"{theme.deniedIcon} Only global administrators can manage appointment types.", ephemeral=True)
            return
    ...
    async def delete(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            with closing(sqlite3.connect("db/shifts.sqlite")) as svs_conn:
...
class ActivityPickerSelect(discord.ui.Select):
    """Dropdown of the alliance's configured appointment types."""

    def __init__(self, labels):
        if labels:
            options = [discord.SelectOption(label=label[:100], value=label) for label in labels[:25]]
            super().__init__(placeholder="Select an appointment type to manage...", options=options)
        else:
            super().__init__(
                placeholder="No appointment types configured yet",
                options=[discord.SelectOption(label="No appointment types", value="none")],
    ...
    async def callback(self, interaction: discord.Interaction):
        await self.view._handle_activity_selection(interaction, self.values[0])
...
class MinisterChannelView(discord.ui.View):
    def __init__(self, bot, cog, is_global: bool = False):
        ...
        # Dynamic dropdown of admin-configured appointment types, replacing the
        # old fixed Construction/Research/Troops Training Day buttons.
        self.add_item(ActivityPickerSelect(self.cog.get_appointment_type_labels()))
    ...
    async def event_archive(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not archive_cog:
            await interaction.response.send_message(f"{theme.deniedIcon} Shift Archive module not found.", ephemeral=True)
            return
    ...
    async def _handle_activity_selection(self, interaction: discord.Interaction, activity_name: str):
        ...
        if not log_guild:
            await interaction.response.send_message(
                "Could not find the shift log server. Make sure the bot is in that server.\n\nIf issue persists, run the `/settings` command --> Other Features --> Shift Scheduling --> Delete Server ID and try again in the desired server",
                ephemeral=True
            )
            return
        ...
        if not channel or not log_channel:
            await interaction.response.send_message(
                f"Could not find {activity_name} channel or log channel. Make sure to select a channel for each appointment type for the bot to send the updated list, and a log channel.\n\nYou can do so by running the `/settings` command --> Other Features --> Shift Scheduling --> Channel Setup",
                ephemeral=True
            )
            return
        ...
        if interaction.guild.id != log_guild.id:
            await interaction.response.send_message(
                ...
                "If you want to change the server, run `/settings` command --> Other Features --> Shift Scheduling --> Delete Server ID and try again in the desired server",
                ephemeral=True
            )
            return
...
class ChannelTypeSelect(discord.ui.Select):
    """Dropdown of configured appointment types, used to pick which one's
    channel to configure (replaces the old fixed Construction/Research/Training
    channel buttons)."""

    def __init__(self, labels):
        if labels:
            options = [discord.SelectOption(label=label[:100], value=label) for label in labels[:25]]
            super().__init__(placeholder="Select an appointment type's channel...", options=options)
        else:
            super().__init__(
                placeholder="No appointment types configured yet",
                options=[discord.SelectOption(label="No appointment types", value="none")],
    ...
    async def callback(self, interaction: discord.Interaction):
        activity_name = self.values[0]
        await self.view._handle_channel_selection(interaction, f"{activity_name} channel", activity_name)
...
class ChannelConfigurationView(discord.ui.View):
    def __init__(self, bot, cog):
        ...
        self.add_item(ChannelTypeSelect(self.cog.get_appointment_type_labels()))

    @discord.ui.button(label="Log Channel", style=discord.ButtonStyle.secondary, emoji=f"{theme.documentIcon}", row=1)
        ...
        class ChannelSelectWithBackView(discord.ui.View):
            ...
            async def back_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        f"Configure channels for shift scheduling:\n\n"
                        f"Channel Types\n"
                        f"{theme.upperDivider}\n\n"
                        f"{theme.settingsIcon} Pick an appointment type below to set the channel that shows its available slots\n\n"
...
class MinisterMenu(commands.Cog):
    def __init__(self, bot):
        ...
        self.svs_conn = sqlite3.connect("db/shifts.sqlite", timeout=30.0, check_same_thread=False)
        self.svs_cursor = self.svs_conn.cursor()

        # Same idempotent table-creation convention as minister_schedule.py; this
        # cog also reads/writes appointment_types directly for the CRUD UI.
    ...
    def get_appointment_type_labels(self):
        """Ordered list of configured appointment-type labels (server-wide, alliance_id IS NULL)."""
        self.svs_cursor.execute(
            "SELECT label FROM appointment_types WHERE alliance_id IS NULL ORDER BY sort_order, id"
        )
        return [row[0] for row in self.svs_cursor.fetchall()]
    ...
    async def show_minister_channel_menu(self, interaction: discord.Interaction):
        ...
        labels = self.get_appointment_type_labels()
        if labels:
            types_block = "".join(
                f"{theme.calendarIcon} **{label}**\n└ Manage {label} appointments\n\n" for label in labels
            )
        else:
            types_block = (
                f"{theme.warnIcon} **No appointment types configured yet**\n"
                f"└ An admin should add some first (Settings → Appointment Types)\n\n"
            )

        embed = discord.Embed(
            title="🗓️ Shift Scheduling",
            description=(
                f"Manage your shift appointments here:\n\n"
                f"**Channel Status**\n"
                f"{theme.upperDivider}\n"
                f"{channel_status}\n"
                ...
                f"└ Save and view past shift schedules\n\n"
    ...
    async def show_channel_setup_menu(self, interaction: discord.Interaction):
        embed = discord.Embed(
            ...
            description=(
                ...
                f"Channel Types\n"
                f"{theme.upperDivider}\n\n"
                f"{theme.settingsIcon} Select an appointment type below to set the channel that shows its available slots\n\n"
    ...
    async def get_channel_status_display(self) -> tuple[str, discord.Color]:
        ...
        minister_schedule_cog = self.bot.get_cog("MinisterSchedule")
        if not minister_schedule_cog:
            return f"{theme.warnIcon} **Shift Schedule module not loaded**\n", discord.Color.red()
        ...
        # Define channels to check - one per admin-configured appointment type, plus the log channel
        channels_config = [
            (f"{label} channel", f"{theme.calendarIcon} {label}")
            for label in self.get_appointment_type_labels()
        ]
        channels_config.append(("minister log channel", f"{theme.listIcon} Log Channel"))
    ...
    async def show_filtered_user_select(self, interaction: discord.Interaction, activity_name: str):
        ...
        if not is_admin:
            await interaction.response.send_message(f"{theme.deniedIcon} You do not have permission to manage shift appointments.", ephemeral=True)
            return
    ...
    async def update_minister_names(self, interaction: discord.Interaction, activity_name: str):
        ...
        if not fids:
            await interaction.response.send_message(f"{theme.deniedIcon} No booked appointments to update for {activity_name}.", ephemeral=True)
            return
    ...
    async def show_clear_channels_selection(self, interaction: discord.Interaction):
        ...
        appointment_labels = self.get_appointment_type_labels()

        class ClearChannelsConfirmView(discord.ui.View):
            def __init__(self, parent_cog, labels):
                super().__init__(timeout=7200)
                self.parent_cog = parent_cog
                self.labels = labels

                options = [
                    discord.SelectOption(label=f"{label} Channel"[:100], value=label, emoji=theme.calendarIcon)
                    for label in labels[:23]
                ]
                options.append(discord.SelectOption(label="Log Channel", value="minister log", emoji=theme.documentIcon))
                options.append(discord.SelectOption(label="All Channels", value="ALL", emoji=theme.trashIcon, description="Clear all channel configurations"))
                ...
                select.callback = self.select_channels
                self.add_item(select)
                ...
                try:
                    ...
                    with closing(sqlite3.connect("db/shifts.sqlite")) as svs_conn:
                        ...
                        for value in select_values:
                            if value == "ALL":
                                # Clear all appointment-type channels
                                for activity in self.labels:
        ...
        await interaction.response.edit_message(embed=embed, view=ClearChannelsConfirmView(self, appointment_labels))

    async def show_appointment_types_menu(self, interaction: discord.Interaction, selected_id: int = None, message: str = None):
        """Admin CRUD screen for appointment types (add/rename/delete/reorder)."""
        schedule_cog = self.bot.get_cog("MinisterSchedule")
        if not schedule_cog:
            await interaction.response.send_message(f"{theme.deniedIcon} Shift Schedule module not found.", ephemeral=True)
            return

        types = schedule_cog.get_appointment_types()

        if types:
            types_block = "\n".join(f"`{i + 1}.` **{label}**" for i, (_tid, label, _sort) in enumerate(types))
        else:
            types_block = f"{theme.warnIcon} No appointment types configured yet - use **Add** below to create one."

        description = ""
        if message:
            description += f"{theme.verifiedIcon} **{message}**\n\n"
        description += (
            f"Define the appointment types your alliance can book shifts for.\n\n"
            f"**Configured Types**\n"
            f"{theme.upperDivider}\n"
            f"{types_block}\n"
            f"{theme.lowerDivider}"
        ...
        embed = discord.Embed(
            title=f"{theme.editListIcon} Appointment Types",
            description=description,
            color=theme.emColor1
        )

        view = AppointmentTypesView(self.bot, self, selected_id)
        await safe_edit_message(interaction, embed=embed, view=view, content=None)

    async def show_delete_appointment_type_confirmation(self, interaction: discord.Interaction, type_id: int, label: str):
        """Confirmation prompt before permanently deleting an appointment type."""
        embed = discord.Embed(
            title=f"{theme.warnIcon} Delete Appointment Type",
            description=(
                f"Are you sure you want to delete **{label}**?\n\n"
                f"{theme.warnIcon} This removes it from the booking menus. Any already-booked "
                f"appointments and channel configuration under this label are preserved for history.\n\n"
                f"This action cannot be undone."
            ),
            color=theme.emColor2
        )
        view = DeleteAppointmentTypeConfirmView(self.bot, self, type_id, label)
        await safe_edit_message(interaction, embed=embed, view=view, content=None)

    async def show_settings_menu(self, interaction: discord.Interaction):
        """Show the shift settings menu"""
        _, is_global, _ = await self.get_admin_permissions(interaction.user.id)
        embed = discord.Embed(
            title=f"{theme.settingsIcon} Shift Settings",
            description=(
    ...
    async def show_activity_selection_for_update(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description="Select which appointment type you want to update names for:",
            color=theme.emColor1
        )
    ...
    async def show_activity_selection_for_clear(self, interaction: discord.Interaction):
        ...
        if not log_channel:
            await interaction.response.send_message(
                f"[Warning] Could not find a log channel. Log channel is needed before clearing the appointment \n\nRun the `/settings` command --> Other Features --> Shift Scheduling --> Channel Setup and choose a log channel", ephemeral=True)
            return

        embed = discord.Embed(
            title=f"{theme.calendarIcon} Delete All Reservations",
            description="Select which appointment type you want to clear reservations for:",
            color=theme.emColor2
        )
    ...
    async def migrate_time_slots(self, interaction: discord.Interaction, old_mode: int, new_mode: int):
        ...
        try:
            ...
            if minister_schedule_cog:
                ...
                # Update all channel messages for every configured appointment type
                for activity_name in self.get_appointment_type_labels():
                    await self.update_channel_message(activity_name)
    ...
    async def show_activity_selection_for_list_type(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=f"Select the type of generated shift list message when adding/removing people:\n\n**Currently showing:** {current_label}",
            color=theme.emColor3
        )
