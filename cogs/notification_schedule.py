...
def _shift_months(dt: datetime, months: int) -> datetime:
    """Add `months` calendar months to dt, clamping the day to the target month's length."""
...
def calculate_next_occurrence(first_occurrence: datetime, recurrence_type: str,
                               recurrence_interval: int, from_date: Optional[datetime] = None) -> Optional[datetime]:
    """
    Generic next-occurrence date math for admin-configured custom events.

    Takes plain (first_occurrence, recurrence_type, recurrence_interval) parameters
    rather than switching on a fixed per-event schedule_type enum, so the same
    function serves every custom_events row.

    Args:
        first_occurrence: The event's first/reference occurrence (timezone-aware, UTC).
        recurrence_type: One of RECURRENCE_TYPES - "daily", "weekly", or "monthly".
        recurrence_interval: How many days/weeks/months between occurrences (>= 1).
        from_date: Calculate the next occurrence at/after this moment (defaults to now, UTC).

    Returns:
        Datetime of the next occurrence at or after from_date, or None for an
        unrecognized recurrence_type.
    """
    ...
    if first_occurrence >= from_date:
        return first_occurrence

    if recurrence_type == "daily":
        days_diff = (from_date - first_occurrence).days
        cycles_passed = days_diff // interval
        next_occurrence = first_occurrence + timedelta(days=cycles_passed * interval)
        while next_occurrence < from_date:
            next_occurrence += timedelta(days=interval)
        return next_occurrence

    if recurrence_type == "weekly":
        cycle_days = interval * 7
        days_diff = (from_date - first_occurrence).days
        cycles_passed = days_diff // cycle_days
        next_occurrence = first_occurrence + timedelta(days=cycles_passed * cycle_days)
        while next_occurrence < from_date:
            next_occurrence += timedelta(days=cycle_days)
        return next_occurrence

    if recurrence_type == "monthly":
        months_diff = (from_date.year - first_occurrence.year) * 12 + (from_date.month - first_occurrence.month)
        cycles_passed = max(0, months_diff // interval)
        next_occurrence = _shift_months(first_occurrence, cycles_passed * interval)
        while next_occurrence < from_date:
            cycles_passed += interval
            next_occurrence = _shift_months(first_occurrence, cycles_passed)
        return next_occurrence
...
class NotificationSchedule(commands.Cog):
    ...
    async def urgency_update_loop(self):
        ...
        while not self.bot.is_closed():
            try:
                # Check if vault_notifications table exists (created by notification_system cog)
                self.cursor.execute("""
                    SELECT name FROM sqlite_master
                    WHERE type='table' AND name='vault_notifications'
                """)
            ...
            async def format_section_with_days(events, show_channel):
                ...
                for date in sorted(days_dict.keys()):
                    ...
                    for event_time, notif in days_dict[date]:
                        line = await self._format_event_line(notif, tz, show_channel, settings.get('use_user_timezone', 0), guild_id)
    ...
    async def _format_event_line(self, notification, timezone_obj, show_channel: bool, use_user_timezone: int = 0, guild_id: int = None) -> str:
        ...
        try:
            ...
            emoji = get_event_icon(event_type, guild_id) if event_type else f"{theme.calendarIcon}"
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"└ Ideal for dedicated event channels (e.g., a single event only)\n"
                    f"{theme.lowerDivider}"
                ),
                color=theme.emColor1
            )
