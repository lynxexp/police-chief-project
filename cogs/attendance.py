...
EVENT_TYPES = ["Vault Trap", "PD Development", "Arms Race", "Officer Program", "Hero Program", "Alliance Faceoff", "State Capitol Siege", "Other"]
...
EVENT_TYPE_ICONS = {
    ...
    "Hero Program": "🦸",
    "Alliance Faceoff": "🛡️",
    "State Capitol Siege": "🏛️",
    "Other": "📋"
}

# OCR db keys → friendly (label, icon); manual events use EVENT_TYPE_ICONS.
# "vault" is the literal event_type/origin string vault_track.py's dedicated
# Vault Trap pipeline writes to attendance_sessions/attendance_records (see
# sync_vault_attendance_event in attendance_ocr_parsers.py). It is NOT the
# "vault_trap" key in attendance_ocr_parsers.EVENT_TYPES -- that one is a
# fingerprint-config entry for a different, currently-unused generic OCR
# pipeline (see the NOTE above EVENT_TYPES there) and never appears as a
# stored event_type. The rest of EVENT_TYPES is manual-entry-only until real
# Police Chief screenshots are available for them (Phase 6 note).
_OCR_EVENT_DISPLAY = {
    "vault": ("Vault Trap", "🔐"),
}
...
def event_type_display(event_type: str) -> tuple:
    ...
    ('vault', from the Vault Trap tracking pipeline) or a manual label
    ('Arms Race')."""
...
# Event types that support legion selection (Legion 1, Legion 2). The
# original game's Foundry/Canyon legion-split mechanic has no confirmed Police Chief
# equivalent yet, so this is left empty (inert) rather than guessed.
LEGION_EVENT_TYPES = []
...
class EditEventDetailsView(discord.ui.View):
    def __init__(self, session_id, session_name, current_event_type, current_event_date, parent_view, is_edit=True, current_event_subtype=None):
        ...
        # Create legion dropdown only for legion events (none configured yet -- see LEGION_EVENT_TYPES)
        self.legion_select = None
    ...
    def update_select_menu(self):
        ...
        for player in current_players:
            if isinstance(player, dict):
                ...
                description = f"ID: {fid} | CO: {FC_LEVEL_MAPPING.get(furnace_lv, str(furnace_lv))}"
            else:
    ...
    async def show_attendance_hub(self, interaction: discord.Interaction, alliance_id=None):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"└ Rank the alliance by no-shows, excuse individual ones\n\n"
    ...
    async def show_attendance_marking(self, interaction: discord.Interaction, alliance_id: int, alliance_name: str, session_name: str, session_id: int = None, is_edit: bool = False, event_type: str = "Other", event_date: datetime = None, event_subtype: str = None):
        ...
        try:
            ...
            with sqlite3.connect('db/users.sqlite') as db:
                cursor = db.cursor()
                cursor.execute("""
                    SELECT fid, nickname, chief_office_lv AS furnace_lv
                    FROM users
            ...
            if is_edit and session_id:
                with sqlite3.connect('db/attendance.sqlite') as db:
                    ...
                    if origin_row and origin_row[0] == 'vault':
                        notice = discord.Embed(
                            title=f"{theme.warnIcon} Managed by Vault Trap Tracking",
                            description=(
                                "This event is created from a Vault Trap submission. "
                                "To change who took part or their damage, edit the hunt "
                                "on the Vault Trap side (Damage review or records); the "
