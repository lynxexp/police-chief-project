...
def format_repeat_interval(repeat_minutes, notification_id=None) -> str:
    ...
    if repeat_minutes == -1:
        ...
        with closing(sqlite3.connect("db/events.sqlite")) as conn:
            cursor = conn.cursor()
            cursor.execute("""
...
class EmbedDataView(discord.ui.View):
    ...
    async def notification_setting(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        with closing(sqlite3.connect("db/events.sqlite")) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT channel_id, hour, minute, description, mention_type, repeat_minutes, next_notification, timezone, notification_type FROM vault_notifications WHERE id = ?",
...
class NotificationEditor(commands.Cog):
    ...
    async def start_edit_process(self, interaction: discord.Interaction, notification_id: int,
        ...
        with closing(sqlite3.connect("db/events.sqlite")) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT channel_id, hour, minute, description, mention_type, repeat_minutes, next_notification, timezone, notification_type, event_type FROM vault_notifications WHERE id = ?",
                (notification_id,))
            result = cursor.fetchone()
            embed_results = None
            if result and "EMBED_MESSAGE" in result[3]:
                cursor.execute(
                    "SELECT title, description, color, image_url, thumbnail_url, footer, author, mention_message FROM vault_notification_embeds WHERE notification_id = ?",
                    (notification_id,))
    ...
    async def update_notification(self, view):
        with closing(sqlite3.connect("db/events.sqlite")) as conn:
            ...
            cursor.execute(
                "UPDATE vault_notifications SET channel_id = ?, channel_name = ?, hour = ?, minute = ?, description = ?, mention_type = ?, repeat_minutes = ?, next_notification = ?, notification_type = ? WHERE id = ?",
            ...
            cursor.execute("SELECT guild_id FROM vault_notifications WHERE id = ?", (view.notification_id,))
            result = cursor.fetchone()
    ...
    async def update_embed_notification(self, view):
        with closing(sqlite3.connect("db/events.sqlite")) as conn:
            ...
            cursor.execute(
                "UPDATE vault_notification_embeds SET title = ?, description = ?, color = ?, image_url = ?, thumbnail_url = ?, footer = ?, author = ?, mention_message = ? WHERE notification_id = ?",
            ...
            cursor.execute("SELECT guild_id, channel_id FROM vault_notifications WHERE id = ?", (view.notification_id,))
            result = cursor.fetchone()
