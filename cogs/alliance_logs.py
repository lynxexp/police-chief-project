    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        if custom_id == "set_log_channel":
            try:
                ...
                def build_channel_picker(picker_interaction: discord.Interaction, alliance_id: int):
                    channel_embed = discord.Embed(
                        ...
                        description=(
                            ...
                            f"**Page:** 1/1\n"
                            f"**Total Channels:** {len(picker_interaction.guild.text_channels)}"
                        ),
                        color=theme.emColor1
                    )
                    ...
                    channels = picker_interaction.guild.text_channels
                    channel_view = PaginatedChannelView(channels, channel_select_callback)
                    return channel_embed, channel_view

                if len(alliances_with_counts) == 1:
                    # Only one alliance managed — skip straight to the channel picker.
                    alliance_id = alliances_with_counts[0][0]
                    channel_embed, channel_view = build_channel_picker(interaction, alliance_id)
                    await interaction.response.send_message(
                        embed=channel_embed,
                        view=channel_view,
                        ephemeral=True
                ...
                async def alliance_callback(select_interaction: discord.Interaction, alliance_view):
                    try:
                        ...
                        channel_embed, channel_view = build_channel_picker(select_interaction, alliance_id)
            ...
            try:
                ...
                def build_removal_confirmation(alliance_id: int):
                    with sqlite3.connect('db/alliance.sqlite', timeout=30.0) as conn:
                        cursor = conn.cursor()
                        cursor.execute("SELECT name FROM alliance_list WHERE alliance_id = ?", (alliance_id,))
                        alliance_name = cursor.fetchone()[0]
                    ...
                    return confirm_embed, confirm_view

                if len(alliances_with_counts) == 1:
                    # Only one alliance has a log channel configured — skip straight to confirmation.
                    alliance_id = alliances_with_counts[0][0]
                    confirm_embed, confirm_view = build_removal_confirmation(alliance_id)
                    await interaction.response.send_message(
                        embed=confirm_embed,
                        view=confirm_view,
                        ephemeral=True
