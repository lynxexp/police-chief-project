...
class MainMenu(commands.Cog):
    ...
    def build_main_menu_embed(self) -> discord.Embed:
        ...
        return discord.Embed(
            ...
            description=(
                ...
                f"└ Admin-configured event reminder calendar\n\n"
                f"{theme.listIcon} **Attendance**\n"
                f"└ Track and export event attendance\n\n"
                f"{theme.chartIcon} **Vault Trap Tracking**\n"
                f"└ Track Vault Trap damage and view statistics\n\n"
                f"{theme.ministerIcon} **Shift Scheduling**\n"
                f"└ Manage alliance shift appointments\n\n"
    ...
    async def show_alliance_management(self, interaction: discord.Interaction, force_picker: bool = False):
        """Entry to Alliances — pick an alliance to manage, or run a bulk
        action across alliances (gated by tier). Auto-selected straight to
        the hub when the admin only manages one alliance; pass
        force_picker=True to show the picker/bulk-action screen anyway
        (used when navigating back from the hub or a bulk-action screen, so
        Add Alliance / Self-Registration / etc. stay reachable)."""
        try:
            ...
            if not force_picker and len(alliances) == 1:
                await self.show_alliance_hub(interaction, alliances[0][0])
                return
    ...
    async def show_alliance_hub(self, interaction: discord.Interaction, alliance_id: int):
        ...
        try:
            ...
            with sqlite3.connect('db/users.sqlite') as db:
                cursor = db.cursor()
                cursor.execute(
                    "SELECT COUNT(*), AVG(chief_office_lv), MAX(chief_office_lv) "
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"└ Chief's Office level and nickname changes per member\n\n"
    ...
    def _fc_label(self, fl: int) -> str:
        """Render a chief_office_lv int as e.g. 'Lv. 8'. Falls back to the int."""
...
class MainMenuView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Vault Trap Tracking",
        emoji=theme.chartIcon,
        style=discord.ButtonStyle.primary,
        custom_id="vault_tracking",
        row=1
    )
    async def vault_tracking_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            vault_cog = self.cog.bot.get_cog("VaultTrack")
            if vault_cog:
                await vault_cog.show_vault_track_menu(interaction)
            else:
                await interaction.response.send_message(
                    f"{theme.deniedIcon} Vault Trap Tracking module not found.",
                    ephemeral=True
                )
        except Exception as e:
            logger.error(f"Error loading Vault Trap Tracking menu: {e}")
            print(f"Error loading Vault Trap Tracking menu: {e}")

    @discord.ui.button(
        label="Capitol War Tracking",
        emoji=theme.chartIcon,
        style=discord.ButtonStyle.primary,
        custom_id="capitol_war_tracking",
        row=1
    )
    async def capitol_war_tracking_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            capitol_cog = self.cog.bot.get_cog("CapitolWar")
            if capitol_cog:
                await capitol_cog.show_capitol_menu(interaction)
            else:
                await interaction.response.send_message(
                    f"{theme.deniedIcon} Capitol War Tracking module not found.",
                    ephemeral=True
                )
        except Exception as e:
            logger.error(f"Error loading Capitol War Tracking menu: {e}")
            print(f"Error loading Capitol War Tracking menu: {e}")
    ...
    async def minister_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if minister_cog:
                ...
                await interaction.response.send_message(
                    f"{theme.deniedIcon} Shift Scheduling module not found.",
                    ephemeral=True
                )
        except Exception as e:
            logger.error(f"Error loading Shift Scheduling menu: {e}")
            print(f"Error loading Shift Scheduling menu: {e}")
...
class AllianceHubView(discord.ui.View):
    """Per-alliance hub. Layout:
      ...
      row 2: Edit Name | Set State | Set Tag | History | Power Rankings
      row 3: State Lock | Auto-remove Transfers | Change Server   (long toggle labels, own row)
    ...
    @discord.ui.button(label="Change Server", emoji=theme.globeIcon,
                       style=discord.ButtonStyle.secondary, row=3)
    async def change_server(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Repoint this alliance to whichever server the click happens on --
        the one manual step needed after moving the bot to a new install
        (Vault Trap/Capitol War history and admin permissions are already
        alliance_id/user_id-scoped and need no changes)."""
        await _route_to_cog(
            interaction, self.cog.bot, "Alliance",
            "show_change_server_for", self.alliance_id,
            missing_label="Alliance",
        )
    ...
    @discord.ui.button(label="Set Tag", emoji=theme.pinIcon,
                       style=discord.ButtonStyle.primary, row=2)
    async def set_tag(self, interaction: discord.Interaction, button: discord.ui.Button):
        await _route_to_cog(
            interaction, self.cog.bot, "Alliance",
            "show_edit_tag_for", self.alliance_id,
            missing_label="Alliance",
        )
    ...
    async def back(self, interaction: discord.Interaction, button: discord.ui.Button):
        # force_picker=True: single-alliance admins would otherwise bounce
        # right back into this same hub, losing access to Add Alliance /
        # Self-Registration / other bulk actions on the entry screen.
        await self.cog.show_alliance_management(interaction, force_picker=True)
...
class ExternalOcrModal(discord.ui.Modal):
    ...
    def __init__(self, cog, is_global: bool):
        ...
        from .vault_track import remote_ocr_setting, OCR_REMOTE_URL_DEFAULT, OCR_REMOTE_URL_ENV
        ...
        self.url_input = discord.ui.TextInput(
            ...
            placeholder="Blank = local OCR. Set to your own remote OCR service URL to use one.",
            default=prefill,
            required=False,
            max_length=300,
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            from .vault_track import invalidate_remote_ocr_cache
...
class MaintenanceView(discord.ui.View):
    ...
    def __init__(self, cog, is_global: bool = False):
        ...
        global_only = {"backup_system", "bot_health", "bot_presence",
                       "toggle_remote_ocr"}
        for child in self.children:
        ...
        try:
            from .vault_track import remote_ocr_url
