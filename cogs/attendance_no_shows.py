"""No-Shows: alliance-wide registered-no-show ranking for events with a
registration+result OCR flow. Police Chief has no confirmed registration-mail
event yet -- Vault Trap OCR (Phase 6) is a summary+rankings mail with no
sign-up phase, so no-show tracking has nothing to attach to right now. Left
inert (empty event list) rather than guessed."""
import asyncio
import sqlite3
...
# No Police Chief event currently has a registration+result OCR pair (see
# module docstring), so this stays empty until one does.
NO_SHOW_EVENT_TYPES = []
_EVENT_LABEL_BY_KEY = {}
...
class NoShowsView(discord.ui.View):
    ...
    def _build_components(self):
        ...
        event_label = _EVENT_LABELS[self._event_idx]
        # No per-event icons defined yet (NO_SHOW_EVENT_TYPES is empty); fall
        # back to a generic icon for every state.
        event_icon = theme.listIcon
