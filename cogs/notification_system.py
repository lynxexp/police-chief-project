...
class QuarantinedNotificationActionView(discord.ui.View):
    ...
    async def _on_reenable(self, interaction: discord.Interaction):
        ...
        try:
            self.cog.cursor.execute(
                "UPDATE vault_notifications SET is_enabled = 1, auto_disabled_at = NULL "
    ...
    async def _on_delete(self, interaction: discord.Interaction):
        ...
        try:
            self.cog.cursor.execute(
                "DELETE FROM vault_notifications "
                "WHERE auto_disabled_at = ? AND guild_id = ? AND is_enabled = 0",
                (self.ts_iso, self.guild_id),
            )
            affected = self.cog.cursor.rowcount
...
class NotificationSystem(commands.Cog):
    def __init__(self, bot):
        ...
        self.cursor.execute("""
            CREATE TABLE IF NOT EXISTS vault_notification_embeds (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                notification_id INTEGER NOT NULL,
                title TEXT,
                description TEXT,
        ...
        try:
            self.cursor.execute("SELECT mention_message FROM vault_notification_embeds LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_notification_embeds ADD COLUMN mention_message TEXT")

        try:
            self.cursor.execute("SELECT event_type FROM vault_notifications LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_notifications ADD COLUMN event_type TEXT")
        try:
            self.cursor.execute("SELECT wizard_batch_id FROM vault_notifications LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_notifications ADD COLUMN wizard_batch_id TEXT")
        try:
            self.cursor.execute("SELECT instance_identifier FROM vault_notifications LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_notifications ADD COLUMN instance_identifier TEXT")
        try:
            self.cursor.execute("SELECT auto_disabled_at FROM vault_notifications LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_notifications ADD COLUMN auto_disabled_at TEXT")

        # Links a notification row to the admin-configured custom_events row it was
        # materialized from (NULL for notifications created outside the calendar wizard).
        try:
            self.cursor.execute("SELECT custom_event_id FROM vault_notifications LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_notifications ADD COLUMN custom_event_id INTEGER")

        # Admin-configured custom event calendar. Ships with zero pre-filled rows; also created in
        # notification_wizard.py, which owns the create/edit/delete UI for it.
        self.cursor.execute("""
        ...
        try:
            self.cursor.execute("SELECT custom_delete_delay_minutes FROM vault_notifications LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_notifications ADD COLUMN custom_delete_delay_minutes INTEGER DEFAULT NULL")

        # Last-known channel name, so quarantine messages can name a deleted channel
        try:
            self.cursor.execute("SELECT channel_name FROM vault_notifications LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_notifications ADD COLUMN channel_name TEXT DEFAULT NULL")
        ...
        try:
            self.cursor.execute("SELECT show_daily_reset_on_schedule FROM vault_trap_settings LIMIT 1")
        except sqlite3.OperationalError:
            self.cursor.execute("ALTER TABLE vault_trap_settings ADD COLUMN show_daily_reset_on_schedule INTEGER DEFAULT 0")

        # Initialize default settings for all guilds with notifications
        self.cursor.execute("""
            INSERT OR IGNORE INTO vault_trap_settings (guild_id, delete_messages_enabled, default_delete_delay_minutes, show_daily_reset_on_schedule)
            SELECT DISTINCT guild_id, 1, 60, 0 FROM vault_notifications
        """)
    ...
    async def _pause_and_notify(self, *, channel_id: int | None = None,
        ...
        try:
            self.cursor.execute(
                ...
                f"channel_id, channel_name FROM vault_notifications "
            ...
            self.cursor.execute(
                f"UPDATE vault_notifications SET is_enabled = 0, auto_disabled_at = ? "
                f"WHERE {where_sql} AND is_enabled = 1",
                (ts_iso, *where_params),
            )
            self.conn.commit()
    ...
    async def _sweep_orphaned_notifications(self):
        ...
        try:
            self.cursor.execute(
                "SELECT DISTINCT guild_id FROM vault_notifications WHERE is_enabled = 1"
            )
    ...
    def _register_quarantine_views(self):
        ...
        try:
            self.cursor.execute(
                "SELECT DISTINCT auto_disabled_at, guild_id FROM vault_notifications "
    ...
    async def save_notification(self, guild_id: int, channel_id: int, start_date: datetime,
                                ...
                                selected_weekdays: list[int] = None, event_type: str = None, wizard_batch_id: str = None, instance_identifier: str = None, skip_board_update: bool = False,
                                custom_event_id: int = None) -> int:
        try:
            embed_data = None
            ...
            self.cursor.execute("""
                ...
                mention_type, repeat_enabled, repeat_minutes, created_by, next_notification, event_type, wizard_batch_id, instance_identifier, channel_name, custom_event_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    async def update_notification(self, notification_id: int, hour: int, minute: int, timezone: str,
        ...
        try:
            ...
            if start_date:
                ...
                self.cursor.execute("SELECT next_notification FROM vault_notifications WHERE id = ?", (notification_id,))
            ...
            if embed_data:
                self.cursor.execute("DELETE FROM vault_notification_embeds WHERE notification_id = ?", (notification_id,))
                await self.save_notification_embed(notification_id, embed_data)
            ...
            if not skip_board_update:
                schedule_cog = self.bot.get_cog("NotificationSchedule")
                if schedule_cog:
                    self.cursor.execute("SELECT guild_id, channel_id FROM vault_notifications WHERE id = ?", (notification_id,))
                    row = self.cursor.fetchone()
                    if row:
    ...
    async def save_notification_embed(self, notification_id: int, embed_data: dict) -> bool:
        try:
            self.cursor.execute("""
                INSERT INTO vault_notification_embeds 
    ...
    async def get_notification_embed(self, notification_id: int) -> dict:
        try:
            self.cursor.execute("""
                ...
                FROM vault_notification_embeds 
            ...
            try:
                ...
                self.cursor.execute("""
                    SELECT id, guild_id, channel_id, hour, minute, timezone, description,
                           ...
                           event_type, instance_identifier, custom_delete_delay_minutes, custom_event_id
    ...
    def calculate_delete_time(self, guild_id: int, event_type: str,
        ...
        # 2. Otherwise fall back to the guild default. (Custom events have no
        #    fixed "duration" field of their own.)
    ...
    async def delete_previous_notifications(self, notification_id: int, channel_id: int,
        ...
        try:
            ...
            if event_type and instance_identifier:
                ...
                self.cursor.execute("""
                    SELECT id FROM vault_notifications
                    WHERE channel_id = ?
                      AND event_type = ?
    ...
    def _next_monthly_custom_event_time(self, custom_event_id: int, now: datetime, hour: int, minute: int):
        """
        Advance a "monthly" custom_events row to its next calendar-month occurrence.

        repeat_minutes can't express this with a fixed period (calendar months
        vary in length), so monthly custom events are flagged with the
        repeat_minutes == -2 sentinel and their next fire time is recomputed
        here via the generalized calculate_next_occurrence() date math instead.
        """
        try:
            self.cursor.execute(
                "SELECT first_occurrence, recurrence_interval FROM custom_events WHERE id = ?",
                (custom_event_id,),
            )
            row = self.cursor.fetchone()
            if not row:
                return now.replace(hour=hour, minute=minute, second=0, microsecond=0) + timedelta(days=1)
            first_occurrence = datetime.fromisoformat(row[0])
            interval = row[1] or 1
            return calculate_next_occurrence(first_occurrence, "monthly", interval, from_date=now)
        except Exception as e:
            logger.error(f"Error advancing monthly custom event {custom_event_id}: {e}")
            return now.replace(hour=hour, minute=minute, second=0, microsecond=0) + timedelta(days=1)
    ...
    async def process_notification(self, notification):
        ...
        try:
            (id, guild_id, channel_id, hour, minute, timezone, description,
             ...
             next_notification, event_type, instance_identifier, custom_delete_delay_minutes,
             custom_event_id) = notification
            ...
            if next_time < now:
                if repeat_enabled:
                    if isinstance(repeat_minutes, int) and repeat_minutes > 0:
                        ...
                        next_time = self._next_monthly_custom_event_time(custom_event_id, now, hour, minute)
            ...
            if should_notify:
                ...
                # Get event emoji from the admin-configured custom event, if any
                event_emoji = ""
                if event_type:
                    from .notification_event_types import get_event_icon
                    event_emoji = get_event_icon(event_type, guild_id)
                ...
                if channel_name:
                    self.cursor.execute(
                        "UPDATE vault_notifications SET channel_name = ? "
    ...
    async def delete_notification(self, notification_id):
        try:
            ...
            self.cursor.execute("""SELECT id, guild_id, channel_id FROM vault_notifications WHERE id = ?""", (notification_id,))
            result = self.cursor.fetchone()
            if not result:
            ...
            self.cursor.execute("""DELETE FROM vault_notifications WHERE id = ?""", (notification_id,))
    ...
    async def toggle_notification(self, notification_id: int, enabled: bool, skip_board_update: bool = False) -> bool:
        try:
            ...
            self.cursor.execute("""
                SELECT is_enabled, guild_id, channel_id FROM vault_notifications WHERE id = ?
            """, (notification_id,))
            result = self.cursor.fetchone()
            if not result:
                return False
...
class RepeatOptionView(discord.ui.View):
    ...
    async def save_notification(self, interaction, repeat, repeat_minutes=0, interval_text=None, selected_weekdays=None):
        try:
            ...
            if self.event_type:
                event_icon = get_event_icon(self.event_type, interaction.guild_id)
...
class EmbedEditorView(discord.ui.View):
    def __init__(self, cog, start_date, hour, minute, timezone, original_message, event_type=None):
        ...
        self.embed_data = {
            "title": f"{theme.alarmClockIcon} Custom Notification",
    ...
    async def update_embed(self, interaction: discord.Interaction):
        try:
            ...
            example_emoji = get_event_icon(self.event_type, interaction.guild_id) if self.event_type else f"{theme.calendarIcon}"
    ...
    async def edit_description(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            modal = TextInputModal(
                ...
                placeholder="Example: Get ready for Vault Trap! Only %t remaining.",
    ...
    async def edit_author(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            modal = TextInputModal(
                title="Edit Author",
                label="Author Text",
                placeholder="Example: Weekly Raid",
...
class EventTypeSelectView(discord.ui.View):
    ...
    def __init__(self, cog, start_date, hour, minute, timezone, original_message, guild_id=None):
        super().__init__(timeout=7200)
        self.cog = cog
        self.start_date = start_date
        self.hour = hour
    ...
    async def continue_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            example_emoji = get_event_icon(self.selected_event_type, self.guild_id) if self.selected_event_type else f"{theme.calendarIcon}"
            example_event_time = f"{self.hour:02d}:{self.minute:02d}"
            example_date = self.start_date.strftime("%b %d") if self.start_date else "Dec 06"
...
class EventTypeDropdown(discord.ui.Select):
    ...
    def __init__(self, parent_view: EventTypeSelectView):
        ...
        # Add this guild's configured custom events, with their icons
        event_types = get_event_types(parent_view.guild_id)
        for event_type in event_types:
            icon = get_event_icon(event_type, parent_view.guild_id)
...
class VaultTrapView(discord.ui.View):
    def __init__(self, cog):
        super().__init__(timeout=None)
        self.cog = cog
        ...
        try:
            ...
            def get_page_option(page):
                ...
                for notif in page_notifications:
                    ...
                    if event_type:
                        from .notification_event_types import get_event_icon
                        event_emoji = get_event_icon(event_type, interaction.guild_id)
                        ...
                        if "EMBED_MESSAGE:" in notification_desc:
                            ...
                            try:
                                self.cog.cursor.execute("""
                                    SELECT title FROM vault_notification_embeds
                                    WHERE notification_id = ?
                                """, (notif[0],))
                        ...
                        async def on_submit(modal_self, interaction: discord.Interaction):
                            ...
                            for n in notifications:
                                if "EMBED_MESSAGE:" in n[6]:
                                    button_self.cog.cursor.execute("SELECT title FROM vault_notification_embeds WHERE notification_id = ?", (n[0],))
                ...
                try:
                            ...
                            try:
                                self.cog.cursor.execute(
                                    """SELECT channel_id, hour, minute, description, mention_type, next_notification, event_type
                                       FROM vault_notifications WHERE id = ?""",
                                ...
                                example_emoji = get_event_icon(event_type, interaction.guild_id) if event_type else f"{theme.calendarIcon}"
                                ...
                                if "EMBED_MESSAGE:" in description:
                                    self.cog.cursor.execute("""
                                        SELECT title, description, color, image_url, thumbnail_url, footer, author, mention_message
                                        FROM vault_notification_embeds WHERE notification_id = ?
                                    """, (self.notification_id,))
                    ...
                    class ToggleButton(discord.ui.Button):
                        def __init__(self, cog, notification_id, edit_button, select):
                            ...
                            self.cog.cursor.execute("""
                                SELECT is_enabled FROM vault_notifications WHERE id = ? 
                            """, (self.notification_id,))
    ...
    async def settings_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if row:
                ...
                self.cursor.execute("""
                    INSERT INTO vault_trap_settings (guild_id, delete_messages_enabled, default_delete_delay_minutes)
                    VALUES (?, ?, ?)
