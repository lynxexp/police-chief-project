...
class Alliance(commands.Cog):
    ...
    async def show_edit_tag_for(self, interaction: discord.Interaction, alliance_id: int):
        """Open the per-alliance short-tag modal. Empty clears the tag.

        The tag is the 3-character bracketed code the game shows before every
        player's name in state-wide ranking screens (e.g. "[APX]Lynx") --
        Capitol War's Honor Roll uses it to filter the state-wide rankings
        down to just this alliance before roster matching (see
        docs/ocr-reference/capitol_war.md). Mirrors show_edit_state_for's
        pattern exactly (single-field modal, re-queries fresh instead of
        trusting a cached value)."""
        with sqlite3.connect('db/alliance.sqlite', timeout=30.0) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT name, tag FROM alliance_list WHERE alliance_id = ?",
                (alliance_id,),
            )
            row = cursor.fetchone()
        if not row:
            ...
            return
        alliance_name, tag = row
        await interaction.response.send_modal(
            EditTagModal(alliance_id, alliance_name, tag, self.conn, self.bot)
        )

    async def show_change_server_for(self, interaction: discord.Interaction, alliance_id: int):
        """Repoint an alliance's discord_server_id to whichever Discord
        server this command is currently run from. This is the one field
        that actually needs manual attention when moving the bot to a new
        install/host -- Vault Trap/Capitol War history (alliance_id-keyed)
        and admin permissions (Discord-user-id-keyed) already carry over
        untouched; nothing else in this bot's data model references which
        server an alliance is on."""
        if interaction.guild is None:
            await interaction.response.send_message(
                f"{theme.deniedIcon} This can only be run in a server, not a DM.",
                ephemeral=True,
            )
            return
        with sqlite3.connect('db/alliance.sqlite', timeout=30.0) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT name, discord_server_id FROM alliance_list WHERE alliance_id = ?",
                (alliance_id,),
            )
            row = cursor.fetchone()
        if not row:
            ...
            return
        alliance_name, old_server_id = row
        new_server_id = interaction.guild.id

        if old_server_id == new_server_id:
            await interaction.response.send_message(
                f"{theme.infoIcon} **{alliance_name}** is already pointed at this server.",
                ephemeral=True,
            )
            return

        old_guild = self.bot.get_guild(int(old_server_id)) if old_server_id else None
        old_label = (
            old_guild.name if old_guild
            else (f"a server the bot has left (ID `{old_server_id}`)" if old_server_id else "no server set")
        )
        new_guild_name = interaction.guild.name

        async def _on_confirm(confirm_interaction: discord.Interaction):
            # Re-verify at click time, not just at menu-entry time -- this
            # prompt can sit open for up to 2 minutes (see the view's
            # timeout), and it's consequential enough to be worth a fresh
            # check, matching the pattern already used for the External
            # OCR Service setting (see tests/test_external_ocr_permission.py).
            if not PermissionManager.can_manage_alliance(
                confirm_interaction.user.id,
                confirm_interaction.guild_id if confirm_interaction.guild else 0,
                alliance_id,
            ):
                await confirm_interaction.response.send_message(
                    f"{theme.deniedIcon} You don't have permission to manage this alliance.",
                    ephemeral=True,
                )
                return
            try:
                with sqlite3.connect('db/alliance.sqlite', timeout=30.0) as conn:
                    conn.execute(
                        "UPDATE alliance_list SET discord_server_id = ? WHERE alliance_id = ?",
                        (new_server_id, alliance_id),
                    )
                    conn.commit()
            except Exception as e:
                logger.error(f"Error changing server for alliance {alliance_id}: {e}")
                print(f"Error changing server for alliance {alliance_id}: {e}")
                await confirm_interaction.response.send_message(
                    f"{theme.deniedIcon} Failed to update the server.", ephemeral=True
                )
                return
            await confirm_interaction.response.edit_message(
                embed=discord.Embed(
                    title=f"{theme.verifiedIcon} Server Updated",
                    description=(
                        f"**{alliance_name}** is now linked to **{new_guild_name}**.\n\n"
                        f"Vault Trap/Capitol War history and admin permissions were "
                        f"already portable and needed no changes. If you haven't "
                        f"already, reconfigure this alliance's tracking/log/redemption "
                        f"channels here via **Channel Setup** -- the old channel IDs "
                        f"won't exist on this server."
                    ),
                    color=theme.emColor3,
                ),
                view=None,
        ...
        async def _on_cancel(confirm_interaction: discord.Interaction):
            await confirm_interaction.response.edit_message(
                embed=discord.Embed(
                    title=f"{theme.infoIcon} No Change Made",
                    description=f"**{alliance_name}** is still linked to {old_label}.",
                    color=theme.emColor1,
                ),
                view=None,
            )

        view = _ChangeServerConfirmView(interaction.user.id, on_confirm=_on_confirm, on_cancel=_on_cancel)
        embed = discord.Embed(
            title=f"{theme.globeIcon} Change Server for {alliance_name}?",
            description=(
                f"**Current:** {old_label}\n"
                f"**New:** {new_guild_name} (this server)\n\n"
                f"{theme.warnIcon} This alliance's Vault Trap, Capitol War, and member "
                f"data won't be touched -- it's tied to the alliance itself, not the "
                f"server. This only changes which server owns/manages this alliance "
                f"record.\n\n"
                f"Already-registered members will still show their old server until "
                f"they re-run `/register` here."
            ),
            color=theme.emColor1,
        )
...
class EditTagModal(discord.ui.Modal):
    """Single-field editor for an alliance's 3-character short tag (e.g.
    "APX"). Case-insensitive in-game -- always stored uppercase so Capitol
    War's tag-filtering only needs a single, case-normalized comparison
    against the bracketed tag OCR reads off the Honor Roll (which can render
    in mixed case, e.g. "[Yns]"). Blank input clears the tag."""

    def __init__(self, alliance_id: int, alliance_name: str, current_tag, conn, bot):
        super().__init__(title="Set Alliance Tag")
        self.alliance_id = alliance_id
        self.alliance_name = alliance_name
        self.conn = conn
        self.bot = bot
        self.tag_input = discord.ui.TextInput(
            label="Tag (3 characters, blank = clear)",
            placeholder="e.g. APX",
            default=(current_tag or ""),
            required=False,
            max_length=3,
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        raw = (self.tag_input.value or "").strip()
        new_tag = None
        if raw:
            if len(raw) != 3 or not raw.isalnum():
                await interaction.response.send_message(
                    f"{theme.deniedIcon} Tag must be exactly 3 letters/numbers "
                    f"(or leave blank to clear).",
                    ephemeral=True,
                )
                return
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE alliance_list SET tag = ? WHERE alliance_id = ?",
                (new_tag, self.alliance_id),
            )
            self.conn.commit()
        except Exception as e:
            logger.error(f"Error setting tag on alliance {self.alliance_id}: {e}")
            print(f"Error setting tag on alliance {self.alliance_id}: {e}")
            try:
                await interaction.response.send_message(
                    f"{theme.deniedIcon} Failed to update tag.", ephemeral=True
                )
            except Exception:
                pass
            return
        ...
        if new_tag is None:
            result = f"{theme.verifiedIcon} **{self.alliance_name}** tag cleared."
        else:
            result = (
                f"{theme.verifiedIcon} **{self.alliance_name}** tag set to `{new_tag}`. "
                f"Capitol War tracking will use it to filter state-wide rankings to "
                f"this alliance."
            )
...
class _ChangeServerConfirmView(discord.ui.View):
    """Generic Yes/No confirmation for show_change_server_for. Mirrors
    bot_main_menu.py's _ConfirmActionView shape (kept local rather than
    cross-imported -- that class is module-private by convention, and
    this codebase already prefers small per-file mirrors over reaching
    into another cog's private helpers, e.g. capitol_war.py's
    CapitolAutoDeleteTracker vs vault_track.py's)."""

    def __init__(self, viewer_id: int, *, on_confirm, on_cancel):
    ...
    @discord.ui.button(label="Confirm", style=discord.ButtonStyle.danger,
                       emoji=theme.verifiedIcon, row=0)
