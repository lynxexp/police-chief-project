"""
Shift archive. Stores and displays historical shift appointment data.
"""
import discord
...
class ArchiveTypeSelect(discord.ui.Select):
    """Dropdown of the appointment types actually present in this archive,
    replacing the old fixed View Construction/Research/Training buttons."""

    def __init__(self, type_counts):
        options = [
            discord.SelectOption(label=f"{atype} ({count})"[:100], value=atype)
            for atype, count in type_counts[:25]
        ]
        super().__init__(placeholder="View appointments for a type...", options=options, row=0)

    async def callback(self, interaction: discord.Interaction):
        await self.view.cog.show_archive_appointments(interaction, self.view.archive_id, self.values[0])
...
class ArchiveDetailsView(discord.ui.View):
    def __init__(self, bot, cog, archive_id, archive_info, type_counts):
        ...
        if type_counts:
            self.add_item(ArchiveTypeSelect(type_counts))
...
class SaveArchiveModal(discord.ui.Modal, title="Save Shift Schedule Archive"):
    def __init__(self, cog):
        super().__init__()
        self.cog = cog

        # Generate default name based on current date
        default_name = datetime.now().strftime("Shifts %Y-%m-%d")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not archive_name:
            archive_name = datetime.now().strftime("Shifts %Y-%m-%d")
...
class MinisterArchive(commands.Cog):
    def __init__(self, bot):
        ...
        # Same idempotent table-creation convention as minister_schedule.py.
        # This cog doesn't read/write appointment_types directly today, but
        # creates it defensively in case this cog loads before minister_schedule.
        self.svs_cursor.execute("""
    ...
    async def show_archive_menu(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"{theme.documentIcon} Shift Schedule Archives",
            description=(
                f"Manage shift schedule archives and change history:\n\n"
                f"Available Operations\n"
                f"{theme.middleDivider}\n\n"
                f"{theme.saveIcon} **Save Current Schedule**\n"
                f"└ Archive current shift appointments\n\n"
    ...
    async def save_current_schedule(self, interaction: discord.Interaction, archive_name: str):
        """Save the current shift schedule to an archive"""
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"{theme.middleDivider}\n\n"
                    f"**Would you like to clear all appointments to prepare for the next shift cycle?**\n\n"
                    f"{theme.warnIcon} This will remove all current appointments across every appointment type."
                ),
    ...
    async def clear_all_after_archive(self, interaction: discord.Interaction, archive_id: int):
        """Clear all appointments after archiving, for whatever appointment types
        are currently present (supports any number of appointment types)."""
        await interaction.response.defer()

        try:
            minister_schedule_cog = self.bot.get_cog("MinisterSchedule")
            if not minister_schedule_cog:
                await interaction.followup.send(f"{theme.deniedIcon} Shift Schedule module not found.", ephemeral=True)
                return

            # Discover whichever appointment types actually have bookings right now,
            # rather than assuming a fixed set of names.
            self.svs_cursor.execute("SELECT DISTINCT appointment_type FROM appointments")
            types_present = [row[0] for row in self.svs_cursor.fetchall()]

            cleared_total = 0
            for appointment_type in types_present:
            ...
            # Update all channel messages for the types that were just cleared
            minister_menu_cog = self.bot.get_cog("MinisterMenu")
            if minister_menu_cog:
                for activity_name in types_present:
                    await minister_menu_cog.update_channel_message(activity_name)

            # Send log to shift log channel
            embed = discord.Embed(
                title=f"{theme.trashIcon} All Shift Appointments Cleared",
                description=f"All {cleared_total} appointments cleared after archiving.\n\nReady for the next shift cycle.",
            ...
            embed = discord.Embed(
                ...
                description=f"Successfully cleared {cleared_total} shift appointments.\n\nThe system is now ready for the next shift cycle.",
                color=theme.emColor3
            )
    ...
    async def show_archive_list(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not archives:
                embed = discord.Embed(
                    title=f"{theme.archiveIcon} Shift Schedule Archives",
