...
def id_format_examples(multistate):
    ...
    if multistate:
        return "`12345678 245`", "`12345678, Name, 12, 245`"
    return "`12345678`", "`12345678, Name, 12`"
...
class AllianceIDChannel(commands.Cog):
    ...
    async def process_fid(self, message, fid, alliance_id, given_state=None,
        ...
        try:
            ...
            chief_office_lv = given_level or 0
            try:
                with sqlite3.connect('db/users.sqlite') as users_db:
                    cursor = users_db.cursor()
                    cursor.execute("SELECT alliance FROM users WHERE fid = ?", (fid,))
            ...
            success_embed = discord.Embed(
                ...
                description=(
                    ...
                    + (f"**{theme.levelIcon} Level:** `{format_furnace_level(chief_office_lv)}`\n" if chief_office_lv else "")
