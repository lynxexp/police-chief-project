...
from .attendance_ocr_parsers import EVENT_TYPES, SESSION_CLASSES

_SETTINGS_DB = "db/settings.sqlite"

# Event types actually wired into this generic multi-image OCR pipeline
# (attendance_ocr.py -> build_session). vault_trap has real OCR support but
# via its own dedicated pipeline (vault_track.py) rather than this generic
# one -- see the NOTE above EVENT_TYPES in attendance_ocr_parsers.py.
# Excluded here so it's never offered as a toggle that would lead to
# "Parser not implemented" on upload.
_TOGGLEABLE_EVENT_TYPES: list[str] = [et for et in EVENT_TYPES if et in SESSION_CLASSES]
...
def find_conflicting_channel_owner(channel_id: int, requesting_alliance_id: int) -> Optional[tuple[str, int, str]]:
    ...
    already claims this channel for OCR (Vault Trap, Capitol War, or
    Screenshot Upload), or None if it's free. Re-picking by the same
    alliance is not a conflict.

    The Capitol War check is wrapped in its own try/except: capitol_war.py
    adds the `capitol_score_channel` column lazily on cog init (same
    migration-guard pattern as every other per-feature column here), so a
    boot order where this runs before that cog has initialized must not
    crash the Vault Trap / Screenshot Upload checks that still work fine.
    """
    with sqlite3.connect("db/alliance.sqlite", timeout=30.0) as conn:
        row = conn.execute(
            "SELECT a.alliance_id, l.name FROM alliancesettings a "
            "LEFT JOIN alliance_list l ON l.alliance_id = a.alliance_id "
            "WHERE a.vault_score_channel = ? AND a.alliance_id != ? LIMIT 1",
            (channel_id, requesting_alliance_id),
        ).fetchone()
        if row:
            return ("Vault Trap Tracking", int(row[0]), row[1] or f"#{row[0]}")
        try:
            row = conn.execute(
                "SELECT a.alliance_id, l.name FROM alliancesettings a "
                "LEFT JOIN alliance_list l ON l.alliance_id = a.alliance_id "
                "WHERE a.capitol_score_channel = ? AND a.alliance_id != ? LIMIT 1",
                (channel_id, requesting_alliance_id),
            ).fetchone()
        except sqlite3.OperationalError:
            row = None  # capitol_score_channel column not created yet
        if row:
            return ("Capitol War Tracking", int(row[0]), row[1] or f"#{row[0]}")
    with sqlite3.connect(_SETTINGS_DB, timeout=30.0) as conn:
        row = conn.execute(
...
# the event_type used in _TOGGLEABLE_EVENT_TYPES. Empty for now -- no event
# type is currently wired into this generic pipeline (vault_trap has real OCR
# support but via its own dedicated pipeline; see _TOGGLEABLE_EVENT_TYPES above).
...
def render_info_message(channel_id: int) -> str:
    ...
    lines.append(
        ...
        "(e.g. every page of a paginated results mail) so the bot can assemble the "
        "full picture before you review it."
    )
...
def _summarize_events(channel_id: int) -> str:
    ...
    if not enabled:
        return "(no events)"
    if len(enabled) == len(_TOGGLEABLE_EVENT_TYPES):
        return "All Events"
...
async def _assign_channel_to_alliance(cog, interaction, *, user_id, guild_id,
    ...
    for et in _TOGGLEABLE_EVENT_TYPES:
...
class OCRChannelEditView(discord.ui.View):
    ...
    def _build_components(self):
        ...
        enabled = self._enabled_events()
        options = [
            discord.SelectOption(label=EVENT_TYPES[et].label[:100], value=et, default=et in enabled)
            for et in _TOGGLEABLE_EVENT_TYPES
        ]
        select = discord.ui.Select(
...
class _KeywordsModal(discord.ui.Modal):
    ...
    def __init__(self, parent: _KeywordsView, *, event_type: str,
        ...
        self.input = discord.ui.TextInput(
            ...
            placeholder="Vault Trap Operation\nVault Trap",
            required=False,
            max_length=500,
        )
