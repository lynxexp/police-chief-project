"""
Shift scheduling logic. Handles booking, swaps, and automatic role assignments
for admin-configurable appointment types.
"""
import discord
...
class ChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        svs_conn = sqlite3.connect("db/shifts.sqlite")
        svs_cursor = svs_conn.cursor()
        try:
            # Check if we're updating a shift channel
            if self.context.endswith("channel"):
                # Get the activity name from the context (e.g., "Rally Lead channel" -> "Rally Lead")
                activity_name = self.context.replace(" channel", "")

                # Check if this is one of the configured appointment-type channels
                svs_cursor.execute("SELECT label FROM appointment_types WHERE alliance_id IS NULL")
                configured_labels = {row[0] for row in svs_cursor.fetchall()}
                if activity_name in configured_labels:
            ...
            if minister_menu_cog and self.context.endswith("channel"):
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        ...
                        f"{theme.settingsIcon} One channel per appointment type shows its available slots\n"
...
class MinisterSchedule(commands.Cog):
    def __init__(self, bot):
        ...
        # Admin-configurable appointment-type labels -- admins define their own
        # types rather than choosing from a fixed list.
        # alliance_id is NULL for the current single shared/server-wide schedule;
        # the column is kept for future per-alliance scoping.
        self.svs_cursor.execute("""
    ...
    async def send_embed_to_channel(self, embed):
        ...
        if log_channel:
            ...
            logger.error("Could not find the shift log channel; please change it to a valid channel")
    ...
    # ---- Appointment type CRUD (admin-configurable shift labels) ----

    def get_appointment_types(self, alliance_id: int = None):
        """Return list of (id, label, sort_order) tuples ordered by sort_order, id."""
        if alliance_id is None:
            self.svs_cursor.execute(
                "SELECT id, label, sort_order FROM appointment_types WHERE alliance_id IS NULL ORDER BY sort_order, id"
            )
        else:
            self.svs_cursor.execute(
                "SELECT id, label, sort_order FROM appointment_types WHERE alliance_id=? ORDER BY sort_order, id",
                (alliance_id,)
            )
        return self.svs_cursor.fetchall()

    def get_appointment_type_labels(self, alliance_id: int = None):
        """Return just the ordered label strings."""
        return [label for _id, label, _sort in self.get_appointment_types(alliance_id)]

    def add_appointment_type(self, label: str, alliance_id: int = None) -> int:
        """Insert a new appointment type at the end of the sort order. Returns its id."""
        self.svs_cursor.execute(
            "SELECT COALESCE(MAX(sort_order), -1) + 1 FROM appointment_types WHERE alliance_id IS ?",
            (alliance_id,)
        )
        next_order = self.svs_cursor.fetchone()[0]
        self.svs_cursor.execute(
            "INSERT INTO appointment_types (alliance_id, label, sort_order) VALUES (?, ?, ?)",
            (alliance_id, label, next_order)
        )
        self.svs_conn.commit()
        return self.svs_cursor.lastrowid

    def rename_appointment_type(self, type_id: int, new_label: str):
        self.svs_cursor.execute(
            "UPDATE appointment_types SET label=? WHERE id=?", (new_label, type_id)
        )
        self.svs_conn.commit()

    def delete_appointment_type(self, type_id: int):
        """Delete the appointment type config row. Existing booked appointments for
        that label are intentionally left untouched (same preserve-history behavior
        used elsewhere when clearing channel configuration)."""
        self.svs_cursor.execute("DELETE FROM appointment_types WHERE id=?", (type_id,))
        self.svs_conn.commit()

    def move_appointment_type(self, type_id: int, direction: str, alliance_id: int = None):
        """Swap sort_order with the previous/next type. direction is 'up' or 'down'."""
        types = self.get_appointment_types(alliance_id)
        ids = [t[0] for t in types]
        if type_id not in ids:
            return
        idx = ids.index(type_id)
        target_idx = idx - 1 if direction == "up" else idx + 1
        if target_idx < 0 or target_idx >= len(types):
            return  # already at the edge

        current_id, _, current_order = types[idx]
        target_id, _, target_order = types[target_idx]

        self.svs_cursor.execute("UPDATE appointment_types SET sort_order=? WHERE id=?", (target_order, current_id))
        self.svs_cursor.execute("UPDATE appointment_types SET sort_order=? WHERE id=?", (current_order, target_id))
        self.svs_conn.commit()
    ...
    async def log_change(self, action_type: str, user, appointment_type: str = None, fid: int = None,
        ...
        Args:
            ...
            appointment_type: The admin-configured appointment type label
            fid: User FID
    ...
    async def appointment_autocomplete(self, interaction: discord.Interaction, current: str):
        try:
            ...
            choices = [discord.app_commands.Choice(name=label, value=label) for label in labels]
            if current:
    ...
    async def choice_autocomplete(self, interaction: discord.Interaction, current: str):
        try:
            choices = [
                discord.app_commands.Choice(name="Show full shift list", value="all"),
    ...
    @discord.app_commands.command(name='shift_add', description='Book an appointment slot for a user.')
    ...
    async def minister_add(self, interaction: discord.Interaction, appointment_type: str, fid: str, time: str):
        ...
        try:
            ...
            if not labels:
                await interaction.followup.send(
                    "No appointment types configured yet - an admin should add some first."
                )
                return
            if appointment_type not in labels:
                await interaction.followup.send(
                    f"`{appointment_type}` is not a configured appointment type. Configured types: {', '.join(labels)}"
                )
                return

            # Check shift and log channels
            ...
            if (not channel or not log_channel) and interaction.guild.id != log_guild.id:
                await interaction.followup.send(
                    f"Shift channels or log channel are missing. This command must be run in the server:`{log_guild}` to configure missing channels.\n\n"
                    f"If you want to change that to another server, run `/settings` --> Other Features --> Shift Scheduling --> Delete Server ID and try again in the desired server"
                )
                return
    ...
    @discord.app_commands.command(name='shift_remove', description='Cancel an appointment slot for a user.')
    @app_commands.rename(fid='id')
    ...
    async def minister_remove(self, interaction: discord.Interaction, appointment_type: str, fid: str):
        ...
        try:
            ...
            if not booking:
                await interaction.followup.send(f"{nickname} is not on the shift list for {appointment_type}.")
                return
    ...
    @discord.app_commands.command(name='shift_clear_all', description='Cancel all appointments for a selected appointment type.')
    ...
    async def minister_clear_all(self, interaction: discord.Interaction, appointment_type: str):
        ...
        labels = self.get_appointment_type_labels()
        if not labels:
            await interaction.followup.send("No appointment types configured yet - an admin should add some first.")
            return
        if appointment_type not in labels:
            await interaction.followup.send(
                f"`{appointment_type}` is not a configured appointment type. Configured types: {', '.join(labels)}"
        ...
        if not log_channel:
            await interaction.followup.send(
                f"[Warning] Could not find a log channel. Log channel is needed before clearing the appointment \n\nRun the `/settings` command --> Other Features --> Shift Scheduling --> Channel Setup and choose a log channel")
            return
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=f"Are you sure you want to remove all appointment slots for: {appointment_type}?\n"
    ...
    @discord.app_commands.command(name='shift_list', description='View the schedule for an appointment type.')
    @app_commands.autocomplete(appointment_type=appointment_autocomplete, all_or_available=choice_autocomplete)
    @app_commands.describe(
        appointment_type="The type of appointment to view.",
    ...
    @discord.app_commands.command(name='shift_archive_save', description='Save current shift schedule to an archive (Global Admin only)')
    ...
    async def minister_archive_save(self, interaction: discord.Interaction, name: str = None):
        ...
        if not minister_menu_cog:
            await interaction.response.send_message(f"{theme.deniedIcon} Shift Menu module not found.", ephemeral=True)
            return
        ...
        if not name:
            name = datetime.now().strftime("Shifts %Y-%m-%d")

        # Save the current schedule
        await archive_cog.save_current_schedule(interaction, name)

    @discord.app_commands.command(name='shift_archive_list', description='View all saved shift archives (Global Admin only)')
    ...
    @discord.app_commands.command(name='shift_archive_history', description='View change history for shift appointments (Global Admin only)')
    @app_commands.describe(
