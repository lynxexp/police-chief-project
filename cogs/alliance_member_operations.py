...
from .alliance_member_edit import (
    ...
    edit_result_embed, enqueue_catchups, is_placeholder_name, reactivate_member,
)
...
def _now_iso() -> str:
    """UTC ISO timestamp, matching the convention used for the other `_at`
    columns on `users` (see discord_id_updated_at's write sites)."""
    return datetime.now(timezone.utc).isoformat(timespec='seconds')

_ID_HEADERS = {"id", "fid", "player id", "player_id"}
_NAME_HEADERS = {"name", "nickname", "player name", "player_name"}
_LEVEL_HEADERS = {"co_level", "co level", "chief_office_lv", "chief's office", "chief office", "level"}
...
class MemberListView(discord.ui.View):
    ...
    def build_embed(self) -> discord.Embed:
        ...
        if self.all_members:
            max_fl = max(m['chief_office_lv'] for m in self.all_members)
            avg_fl = sum(m['chief_office_lv'] for m in self.all_members) / all_count
        ...
        if not page_items:
            ...
            for offset, m in enumerate(page_items, start=start + 1):
                level = self.cog.level_mapping.get(m['chief_office_lv'], str(m['chief_office_lv']))
    ...
    def _reload_members(self):
        ...
        try:
            with sqlite3.connect('db/users.sqlite', timeout=30.0) as db:
                rows = db.execute(
                    "SELECT fid, nickname, chief_office_lv, kid FROM users WHERE alliance = ?",
        ...
        for m in self.all_members:
            if m['fid'] in fresh:
                m['nickname'], m['chief_office_lv'], m['kid'] = fresh[m['fid']]
    ...
    async def _on_edit_selected(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_modal(MemberEditModal(
            self, fid, member['nickname'], member['chief_office_lv'], self.alliance_id,
            kid=member.get('kid')))
...
class ManageMembersView(MemberListView):
    ...
    # Cycle order for the Active/Deactivated/All toggle button.
    ACTIVITY_FILTERS = ["active", "deactivated", "all"]
    ACTIVITY_FILTER_LABELS = {"active": "Active", "deactivated": "Deactivated", "all": "All"}

    def __init__(self, members, alliance_id, alliance_name, cog, author_id,
                 alliances=None):
        self.pending_selections: set = set()
        self.alliances = alliances or []  # [(aid, name, count?), ...] for Transfer
        # `members` is queried by show_manage_members_for using this same
        # default (active-only) — kept in sync so a page reload or filter
        # toggle re-queries with the matching WHERE clause.
        self.activity_filter = "active"
        super().__init__(members, alliance_id, alliance_name, cog, author_id)

    @property
    def members(self):
        """3-tuple compatibility shim for IDMultiSelectModal."""
        return [(m['fid'], m['nickname'], m['chief_office_lv']) for m in self.all_members]
    ...
    def _build_components(self):
        ...
        if self.activity_filter == "deactivated":
            reactivate_btn = discord.ui.Button(
                label=f"Reactivate ({len(self.pending_selections)})" if has_selection else "Reactivate",
                emoji=theme.refreshIcon,
                style=discord.ButtonStyle.success,
                disabled=not has_selection,
                row=3,
            )
            reactivate_btn.callback = self._on_reactivate_selected
            self.add_item(reactivate_btn)
        else:
            remove_btn = discord.ui.Button(
                label=f"Deactivate ({len(self.pending_selections)})" if has_selection else "Deactivate",
        ...
        # Row 4: bulk edit, active/deactivated/all toggle, back to alliance hub
        ...
        toggle_btn = discord.ui.Button(
            label=f"View: {self.ACTIVITY_FILTER_LABELS[self.activity_filter]}",
            emoji=theme.eyeIcon,
            style=discord.ButtonStyle.secondary, row=4,
        )
        toggle_btn.callback = self._on_toggle_activity
    ...
    def _query_members(self):
        """Re-fetch from `users` using the current Active/Deactivated/All filter."""
        where = "alliance = ?"
        params: list = [self.alliance_id]
        if self.activity_filter == "active":
            where += " AND is_active = 1"
        elif self.activity_filter == "deactivated":
            where += " AND is_active = 0"
        with sqlite3.connect('db/users.sqlite') as db:
            return db.execute(
                f"SELECT fid, nickname, chief_office_lv, kid FROM users WHERE {where}",
                params,
            ).fetchall()

    async def _on_toggle_activity(self, interaction: discord.Interaction):
        idx = self.ACTIVITY_FILTERS.index(self.activity_filter)
        self.activity_filter = self.ACTIVITY_FILTERS[(idx + 1) % len(self.ACTIVITY_FILTERS)]
        self.all_members = [
            {'fid': fid, 'nickname': nick or '', 'chief_office_lv': fl or 0, 'kid': kid}
            for fid, nick, fl, kid in self._query_members()
        ]
        self._load_power()
    ...
    async def _on_reactivate_selected(self, interaction: discord.Interaction):
        if not self.pending_selections:
            return
        fids = list(self.pending_selections)
        reactivated_rows = [m for m in self.all_members if m['fid'] in set(fids)]
        try:
            with sqlite3.connect('db/users.sqlite') as users_db:
                cur = users_db.cursor()
                placeholders = ",".join("?" * len(fids))
                cur.execute(
                    f"UPDATE users SET is_active = 1, deactivated_at = NULL "
                    f"WHERE fid IN ({placeholders})", fids
                )
                users_db.commit()
            logger.info(
                f"Reactivate: {len(fids)} members in {self.alliance_name}"
            )
            await _post_alliance_log(
                self.cog.bot, self.alliance_id,
                _member_action_log_embed(
                    f"{theme.refreshIcon} Member(s) Reactivated",
                    self.alliance_name, interaction.user, reactivated_rows,
                ),
            )
            reactivated = {m['fid'] for m in reactivated_rows}
            if self.activity_filter == "deactivated":
                self.all_members = [
                    m for m in self.all_members if m['fid'] not in reactivated
                ]
            self.pending_selections.clear()
            self._build_components()
            await interaction.response.edit_message(
            ...
            await interaction.response.send_message(
                f"{theme.deniedIcon} Failed to reactivate members: {e}", ephemeral=True,
            )
    ...
    async def _on_import(self, interaction: discord.Interaction):
        ...
        instructions = discord.Embed(
            ...
            description=(
                ...
                f"• Add a **`Name`** and/or **`CO Level`** column to set them. Members already "
                f"in the alliance get updated, new ones are added already named.\n"
                f"• Levels are plain numbers, e.g. `12` (range 1-45).\n"
        ...
        # Any fid already in the DB (whether or not it had a name/level/state to
        # update above) reactivates if it had been deactivated — re-appearing in
        # an import is exactly the "re-added" signal per decision #1.
        existing_fids_in_file = [f for f in fids if f in known]
        reactivated = await asyncio.to_thread(
            self._reactivate_existing_fids, existing_fids_in_file)
        ...
        if updated or errors or reactivated:
    ...
    def _reactivate_existing_fids(self, fids: list) -> int:
        """Reactivate any of these already-known fids that were deactivated.
        Used by CSV import: a member reappearing in an import file is treated
        the same as a re-add (decision #1), whether or not the file also
        carries a name/level/state change for them."""
        if not fids:
            return 0
        count = 0
        try:
            with sqlite3.connect('db/users.sqlite', timeout=30.0) as conn:
                for fid in fids:
                    if reactivate_member(fid, conn=conn):
                        count += 1
                conn.commit()
        except sqlite3.Error as e:
            logger.warning(f"Member import: reactivation pass failed: {e}")
        return count
    ...
    async def _on_remove_selected(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                title=f"{theme.warnIcon} Confirm Deactivation",
                description=(
                    f"This will deactivate **{len(self.pending_selections)}** member(s) "
                    f"from **{self.alliance_name}**.\n\n"
                    f"Their trap/attendance history is preserved, and they can be "
                    f"reactivated later — automatically if they re-register or are "
                    f"re-added, or by hand via **Reactivate** in Manage Members."
                ),
...
async def _strip_alliance_role(bot, alliance_id, fid) -> None:
    """Remove a deactivated/removed member's alliance tag role (e.g.
    "APX", auto-assigned on /register -- see cogs/alliance_registration.py's
    _apply_registration_roles). Deliberately does NOT touch the generic
    "Registered" role: that one reflects having an in-game ID linked to
    Discord at all, which is independent of any single alliance, and stays
    even if they're removed from this alliance -- see the alliance/user's
    own decision on this. Best-effort and silent on every failure mode
    (member left the server, bot lacks Manage Roles, role positioned above
    the bot's own top role, alliance has no tag configured) -- this must
    never interrupt the deactivation it's cleaning up after."""
    try:
        with sqlite3.connect('db/users.sqlite') as conn:
            row = conn.execute(
                "SELECT discord_id, discord_server_id FROM users WHERE fid = ?", (fid,)
            ).fetchone()
        if not row or not row[0] or not row[1]:
            return
        discord_id, discord_server_id = int(row[0]), int(row[1])
        ...
        tag = (arow[0] or "").strip() if arow else ""
        if not tag:
            return
        ...
        await member.remove_roles(role, reason="Removed/deactivated from alliance")
    except discord.Forbidden:
        # member/guild may not be bound yet if the Forbidden somehow came
        # from an earlier step -- fid/alliance_id are always safe to log.
        logger.warning(
            f"Cannot remove alliance role for fid {fid} (alliance {alliance_id}) -- "
            f"check the bot's role position and Manage Roles permission."
        )
    except Exception as e:
        logger.warning(f"Failed to strip alliance role for fid {fid}: {e}")
...
class _RemoveSelectedConfirmView(discord.ui.View):
    """Yes/no confirmation for ManageMembersView's Deactivate Selected action."""

    def __init__(self, parent_view: ManageMembersView):
        super().__init__(timeout=60)
        self.parent_view = parent_view

    @discord.ui.button(label="Confirm Deactivate", emoji=theme.minusIcon,
                       style=discord.ButtonStyle.danger)
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            with sqlite3.connect('db/users.sqlite') as users_db:
                ...
                placeholders = ",".join("?" * len(fids))
                cur.execute(
                    f"UPDATE users SET is_active = 0, deactivated_at = ? "
                    f"WHERE fid IN ({placeholders})", [_now_iso(), *fids]
                )
                users_db.commit()
            logger.info(
                f"Bulk deactivate: {len(fids)} members from "
                f"{self.parent_view.alliance_name}"
            )
            for fid in fids:
                await _strip_alliance_role(
                    self.parent_view.cog.bot, self.parent_view.alliance_id, fid)
            await _post_alliance_log(
                self.parent_view.cog.bot, self.parent_view.alliance_id,
                _member_action_log_embed(
                    f"{theme.trashIcon} Bulk Member Deactivation",
                    self.parent_view.alliance_name, interaction.user, removed_rows,
                ),
            )
            # Deactivated members drop out of the (default active-only) list
            # this view is currently showing, unless it's already viewing
            # Deactivated/All — in which case they simply stay listed.
            if self.parent_view.activity_filter == "active":
            ...
            await interaction.response.edit_message(
                embed=discord.Embed(
                    title=f"{theme.verifiedIcon} Members Deactivated",
                    description=(
                        f"Deactivated **{len(fids)}** member(s) from "
                        f"**{self.parent_view.alliance_name}**.\n\n"
                        f"Their trap/attendance history is preserved, and they can "
                        f"be reactivated later."
                    ),
                    color=theme.emColor3,
                ),
                view=None,
            ...
            await interaction.response.edit_message(
                embed=discord.Embed(
                    title=f"{theme.deniedIcon} Error",
                    description=f"Failed to deactivate members: {e}",
                    color=theme.emColor2,
                ),
                view=None,
            )
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=discord.Embed(
                title=f"{theme.deniedIcon} Deactivation Cancelled",
                description="No members were deactivated.",
                color=theme.emColor4,
            ),
            view=None,
        )
...
class AllianceMemberOperations(commands.Cog):
    ...
    async def _auto_link_vault_history(self, alliance_id, fid, nickname: str) -> int:
        """Best-effort: link a brand-new member's historical unmatched Vault
        ...
        if vault_cog is None:
            return 0
        try:
            return await vault_cog.auto_link_unmatched_by_name(int(alliance_id), int(fid), nickname)
        except Exception as e:
            logger.warning(f"Vault Trap auto-link failed for new member {fid}: {e}")
            return 0

    async def _auto_link_capitol_history(self, alliance_id, fid, nickname: str) -> int:
        ...
        if capitol_cog is None:
            return 0
        try:
            return await capitol_cog.auto_link_unmatched_by_name(int(alliance_id), int(fid), nickname)
        except Exception as e:
    ...
    async def show_manage_members_for(self, interaction: discord.Interaction, alliance_id: int):
        ...
        with sqlite3.connect('db/users.sqlite') as users_db:
            ...
            cursor.execute(
                "SELECT fid, nickname, chief_office_lv, kid FROM users "
    ...
    async def show_export_members(self, interaction: discord.Interaction):
        ...
        Auto-selects straight to the column picker when the admin only
        manages one alliance (the "ALL ALLIANCES" option is meaningless
        with just one, so it's skipped too)."""
        try:
    ...
    async def show_transfer_members(self, interaction: discord.Interaction):
        ...
        Transfer needs a source AND a destination alliance, so — unlike the
        other pickers — a single managed alliance can't be auto-selected
        (there'd be nowhere to transfer to); show a clear message instead."""
        try:
            alliances, is_global = PermissionManager.get_admin_alliances(
                interaction.user.id,
                interaction.guild_id
            ...
            if len(alliances) == 1:
                await interaction.response.send_message(
                    f"{theme.deniedIcon} Transfer Members requires at least 2 alliances. "
                    f"You currently manage only **{alliances[0][1]}** — add another alliance first.",
                    ephemeral=True
                )
                return
            ...
            async def source_callback(source_interaction: discord.Interaction):
                try:
                    ...
                    with sqlite3.connect('db/users.sqlite') as users_db:
                        cursor = users_db.cursor()
                        cursor.execute("""
                            SELECT fid, nickname, chief_office_lv
                            FROM users
                            WHERE alliance = ?
                            ORDER BY chief_office_lv DESC, nickname
                        """, (source_alliance_id,))
                        members = cursor.fetchall()
    ...
    async def _process_add_user(self, message: Optional[discord.Message], alliance_id: str, alliance_name: str,
        ...
        reactivated_on_add = 0
        for fid in ids_list:
            self.c_users.execute(
                "SELECT nickname, alliance, is_active FROM users WHERE fid=?", (fid,))
            ...
            if existing:
                ...
                if alliance_val and self.c_alliance.fetchone() is None:
                    ...
                    # Already a member somewhere — reactivate if it had been
                    # deactivated (re-adding it is exactly that signal, decision #1),
                    # but it's still reported as "Already Exists" below either way.
                    if not existing[2] and reactivate_member(fid, conn=self.conn_users):
                        self.conn_users.commit()
                        reactivated_on_add += 1
                    already_in_db.append((fid, existing[0]))
            else:
                fids_to_process.append(fid)
        if reactivated_on_add:
            logger.info(
                f"member_add: reactivated {reactivated_on_add} deactivated "
                f"member(s) already present in alliance {alliance_id}"
            )
        ...
        try:
            ...
            while index < len(fids_to_process):
                ...
                try:
                    ...
                    chief_office_lv = parse_furnace_level(csv_level) or 0
                    try:  # Pre-filtered, so this ID should not already exist.
                        self.c_users.execute("""
                            INSERT INTO users (fid, nickname, chief_office_lv, kid, alliance)
                            VALUES (?, ?, ?, ?, ?)
                        """, (fid, nickname, chief_office_lv, kid, alliance_id))
                        self.conn_users.commit()

                        with open(self.log_file, 'a', encoding='utf-8') as f:
                            f.write(f"[{timestamp}] Successfully added member - ID: {fid}, State: {kid}\n")

                        linked_vault_rows += await self._auto_link_vault_history(alliance_id, fid, nickname)
                        linked_capitol_rows += await self._auto_link_capitol_history(alliance_id, fid, nickname)
        ...
        vault_link_note = (
            f"{theme.verifiedIcon} Linked {linked_vault_rows} historical Vault Trap record(s).\n"
            if linked_vault_rows else ""
        )
        capitol_link_note = (
            f"{theme.verifiedIcon} Linked {linked_capitol_rows} historical Capitol War record(s).\n"
            if linked_capitol_rows else ""
        )
        ...
        embed.description = (
            ...
            f"{vault_link_note}{capitol_link_note}\n"
        )
    ...
    async def process_member_export(self, interaction: discord.Interaction, alliance_id, alliance_name: str, selected_columns: list, export_format: str, prefiltered_members: list | None = None):
        ...
        try:
            ...
            if prefiltered_members is not None:
                ...
                # prefiltered_members (from MemberListView.all_members) only carries
                # fid/nickname/chief_office_lv/kid — columns like power/combat_power
                # aren't present there, so use .get() rather than [] to avoid a
                # KeyError that aborted the whole export before any row was written.
                members = [
                    tuple(m.get(col) for col in db_columns)
                ...
                with sqlite3.connect('db/users.sqlite') as users_db:
                    ...
                    query = f"""
                        ...
                        ORDER BY a.name, u.chief_office_lv DESC, u.nickname
                    """
                ...
                with sqlite3.connect('db/users.sqlite') as users_db:
                    ...
                    query = f"SELECT {', '.join(db_columns)} FROM users WHERE alliance = ? ORDER BY chief_office_lv DESC, nickname"
            ...
            for member in members:
                ...
                for i, (db_col, header) in enumerate(columns_to_use):
                    ...
                    # Special formatting for CO Level
                    if db_col == 'chief_office_lv' and value is not None:
            ...
            try:
                ...
                # Add statistics
                if 'chief_office_lv' in db_columns:
                    co_index = db_columns.index('chief_office_lv')
                    co_levels = [m[co_index] for m in members if m[co_index] is not None]
                    if co_levels:
                        max_co = max(co_levels)
                        avg_co = sum(co_levels) / len(co_levels)
                        dm_embed.add_field(
                            name=f"{theme.chartIcon} Statistics",
                            value=(
                                f"**Highest CO:** {self.level_mapping.get(max_co, str(max_co))}\n"
                                f"**Average CO:** {self.level_mapping.get(int(avg_co), str(int(avg_co)))}"
                            ),
                            inline=False
                        )
                ...
                # `file`'s underlying stream was already read to EOF while attempting
                # the DM send above (discord.py reads it to build that request's
                # multipart payload before Discord's API even responds), so reusing
                # it here would attach an empty file. Build a fresh one instead.
                retry_file = discord.File(io.BytesIO(file_content.encode('utf-8')), filename=filename)
                await interaction.followup.send(file=retry_file, ephemeral=True)
                return
...
class AllianceSelectView(discord.ui.View):
    def __init__(self, alliances_with_counts, cog=None, page=0, context="transfer"):
        ...
        self.context = context  # "transfer", "chief_office_history", or "nickname_history"
...
class IDSearchModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if self.context in ["chief_office_history", "nickname_history"]:
                ...
                if changes_cog:
                    ...
                    if self.context == "chief_office_history":
                        await changes_cog.show_chief_office_history(interaction, int(fid))
                    else:
            ...
            with sqlite3.connect('db/users.sqlite') as users_db:
                cursor = users_db.cursor()
                cursor.execute("""
                    SELECT fid, nickname, chief_office_lv, alliance
                    FROM users
                    WHERE fid = ?
                """, (fid,))
                ...
                fid, nickname, chief_office_lv, current_alliance_id = user_result
                ...
                if self.context == "remove":
                    embed = discord.Embed(
                        title=f"{theme.verifiedIcon} Member Found - Deactivate Process",
                        description=(
                            f"**Member Information:**\n"
                            f"{theme.userIcon} **Name:** {nickname}\n"
                            f"{theme.fidIcon} **ID:** {fid}\n"
                            f"{theme.levelIcon} **Level:** {self.cog.level_mapping.get(chief_office_lv, str(chief_office_lv))}\n"
                            f"{theme.allianceIcon} **Current Alliance:** {current_alliance_name}\n\n"
                            f"{theme.warnIcon} **Are you sure you want to deactivate this member?**\n"
                            f"Their trap/attendance history is preserved, and they can be "
                            f"reactivated later."
                        ),
                        color=theme.emColor4
                    ...
                    confirm_button = discord.ui.Button(
                        label=f"{theme.verifiedIcon} Confirm Deactivate",
                        style=discord.ButtonStyle.danger
                    )
                    ...
                    async def confirm_callback(confirm_interaction: discord.Interaction):
                        try:
                            ...
                            await _strip_alliance_role(self.cog.bot, current_alliance_id, fid)

                            success_embed = discord.Embed(
                                title=f"{theme.verifiedIcon} Member Deactivated",
                                description=(
                            ...
                            logger.info(f"Member deactivated via ID search: {nickname} (ID: {fid}) from {current_alliance_name}")
                            ...
                            error_embed = discord.Embed(
                                title=f"{theme.deniedIcon} Error",
                                description="An error occurred during the deactivate operation.",
                                color=theme.emColor2
                            )
                            await confirm_interaction.response.edit_message(
                                embed=error_embed,
                    ...
                    async def cancel_callback(cancel_interaction: discord.Interaction):
                        cancel_embed = discord.Embed(
                            title=f"{theme.deniedIcon} Deactivation Cancelled",
                            description="Member was not deactivated.",
                            color=theme.emColor4
                        )
...
class ExportColumnSelectView(discord.ui.View):
    def __init__(self, alliance_id, alliance_name, cog, include_alliance=False,
        ...
        co_btn = discord.ui.Button(label=f"{theme.verifiedIcon} CO Level", style=discord.ButtonStyle.primary, custom_id="toggle_co", row=0)
        co_btn.callback = self.toggle_co_button
    ...
    def update_buttons(self):
        ...
        for item in self.children:
            if isinstance(item, discord.ui.Button):
                ...
                elif item.custom_id == 'toggle_co':
                    item.style = discord.ButtonStyle.primary if self.selected_columns['co_level'] else discord.ButtonStyle.secondary
                    item.label = f"{theme.verifiedIcon} CO Level" if self.selected_columns['co_level'] else f"{theme.deniedIcon} CO Level"
    ...
    async def toggle_co_button(self, interaction: discord.Interaction):
        self.selected_columns['co_level'] = not self.selected_columns['co_level']
        self.update_buttons()
        
        if not any(self.selected_columns.values()):
            self.selected_columns['co_level'] = True
            self.update_buttons()
            await interaction.response.edit_message(
                content=f"{theme.warnIcon} At least one column must be selected!",
                view=self
    ...
    async def next_button(self, interaction: discord.Interaction):
        ...
        if self.selected_columns['co_level']:
            columns.append(('chief_office_lv', 'CO Level'))
...
class MemberSelectView(discord.ui.View):
    ...
    def update_select_menu(self):
        ...
        for fid, nickname, chief_office_lv in current_members:
            # Mark as default if already selected
            is_selected = int(fid) in self.pending_selections
            options.append(discord.SelectOption(
                label=f"{nickname[:50]}",
                value=str(fid),
                description=f"ID: {fid} | CO: {self.cog.level_mapping.get(chief_office_lv, str(chief_office_lv))}",
                emoji=theme.verifiedIcon if is_selected else theme.userIcon,
                default=is_selected
            ))
