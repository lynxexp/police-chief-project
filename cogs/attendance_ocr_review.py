...
async def _add_unknown_fid(interaction, view, fid):
    """Add an unknown fid straight from the review screen with a placeholder name. Returns
    (added, nickname, note). Never moves a player from another alliance.
    ...
    and assign the alliance's known home state directly."""
    ...
    if alliance_kid is None:
        return False, None, (f"{theme.warnIcon} This alliance has no state set, so I can't add ID `{fid}` "
                             f"without one. Set its state first (Alliance Management → Set State).")
    kid = alliance_kid
    nick = f"Player {fid}"
    with sqlite3.connect("db/users.sqlite", timeout=30.0) as conn:
        conn.execute(
