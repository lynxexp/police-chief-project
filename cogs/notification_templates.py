...
class NotificationTemplates(commands.Cog):
    ...
    def _sync_default_templates(self):
        """
        No-op. Police Chief has no global event registry - the
        notification_templates table ships empty and templates are purely
        admin-authored, so there is nothing to sync from.
        """
        return

    def _populate_default_templates(self):
        """
        No-op. This bot ships with zero pre-filled events for Police Chief,
        so there are no defaults to populate; the templates browser simply
        starts empty until an admin customizes one.
        """
        return
    ...
    def reset_template_to_default(self, template_id: int, event_type: str) -> bool:
        ...
        There is no more global default to reset to - this bot ships with
        zero pre-filled events for Police Chief, so every template is
        admin-authored. Always returns False (nothing to reset).
        """
        return False
...
class TemplatePreviewView(discord.ui.View):
    ...
    async def show_preview(self, interaction: discord.Interaction):
        ...
        sample_emoji = get_event_icon(template["event_type"])
        sample_time = "14:00"
        sample_date = "Nov 29"
