...
class AttendanceOCR(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Keyword gate (mirrors vault_track): if set, only process uploads whose
        ...
        async with lock:
            ...
            # classification OCR (mirrors vault_track's "collecting" message). It's reused
    ...
    async def _classify_images(
        ...
        for attachment in images:
            try:
                ...
                text = await vault_track.ocr_bytes(data, lang=vault_track.DEFAULT_OCR_LANG)
            except Exception:
